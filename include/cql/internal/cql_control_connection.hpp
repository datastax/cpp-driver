...
class cql_control_connection_t : private boost::noncopyable {
public:
    cql_control_connection_t(cql_cluster_t&                         cluster,
    ...
    virtual ~cql_control_connection_t();
    
private:

    void
    metadata_hosts_event(void* sender, boost::shared_ptr<cql_event_t> event);
    ...
    void
    conn_cassandra_event(void* sender, boost::shared_ptr<cql_event_t> event);

    void
    setup_control_connection(bool refresh_only = false);
