...
public:
  ...
  void append(Collection collection) {
    ...
    ASSERT_EQ(CASS_OK, cass_collection_append_dse_line_string(collection.get(),
                                                              line_string.get()));
  }
   ...
   * @return -1 if LHS < RHS, 1 if LHS > RHS, and 0 if equal
   */
  int compare(const LineString& rhs) const {
  ...
  void set(Tuple tuple, size_t index) {
    Native line_string = to_native();
    ASSERT_EQ(CASS_OK, cass_tuple_set_dse_line_string(tuple.get(),
                                                      index,
                                                      line_string.get()));
  }
  ...
  void set(UserType user_type, const std::string& name) {
    Native line_string = to_native();
    ASSERT_EQ(CASS_OK, cass_user_type_set_dse_line_string_by_name(user_type.get(),
                                                                  name.c_str(),
                                                                  line_string.get()));
  }
  ...
  void statement_bind(Statement statement, size_t index) {
    Native line_string = to_native();
    ASSERT_EQ(CASS_OK, cass_statement_bind_dse_line_string(statement.get(),
                                                           index,
                                                           line_string.get()));
  }
  ...
  void statement_bind(Statement statement, const std::string& name) {
    Native line_string = to_native();
    ASSERT_EQ(CASS_OK, cass_statement_bind_dse_line_string_by_name(statement.get(),
                                                                   name.c_str(),
                                                                   line_string.get()));
  }
  ...
  std::string str() const {
    ...
    for (std::vector<Point>::const_iterator iterator = points_.begin();
  ...
  Native to_native() const {
    ...
    if (points_.size() > 0) {
      ...
      dse_line_string_reserve(line_string.get(),
                              static_cast<cass_uint32_t>(points_.size()));

      // Add all the points to the native driver object
      for (std::vector<Point>::const_iterator iterator = points_.begin();
        iterator < points_.end(); ++iterator) {
        Point::PointType point = (*iterator).value();
        EXPECT_EQ(CASS_OK, dse_line_string_add_point(line_string.get(),
                                                     point.x,
                                                     point.y))
