...
class ListSpec extends DataTypeSpec with TableDrivenPropertyChecks  {
  ...
  forAll(protocolVersions) { (protocolVersion: ProtocolVersion) =>
    ...
    "codec with " + protocol must "encode list<varchar> from a List" in {
      codec.encode(List("one", "two", "three")).require.bytes shouldEqual expectedBytes
    }

    it must "encode Nil properly" in {
    ...
    it must "encode list<list<varchar>> from a List" in {
      nestedCodec.encode(List(List("one", "two", "three"), List("four", "five", "six"))).require.bytes shouldEqual expectedNestedBytes
    }

    it must "fail to encode type that is not a List" in {
    ...
    it must "encode and decode back as List" in {
      encodeAndDecode(codec, Nil)
      encodeAndDecode(codec, List("one", "two", "three"))
      encodeAndDecode(codec, Set("one", "two", "three"), List("one", "two", "three"))
