...
public:
  ...
  struct Cluster {
    struct DataCenter {
        ...
        Node(const rapidjson::Value& node, unsigned int dc_id)
          ...
          if (node.IsObject()) {
            if (node.HasMember("name") && node.HasMember("id") &&
                node.HasMember("active_connections") && node.HasMember("address") &&
                node.HasMember("peer_info")) {
              name = node["name"].GetString();
              id = node["id"].GetUint();
              active_connections = node["active_connections"].GetUint();

              const rapidjson::Value& address = node["address"];
              std::vector<std::string> tokens = test::Utils::explode(address.GetString(), ':');
              if (tokens.size() == 2) {
                ip_address = tokens[0];
            ...
            if (data_center.HasMember("nodes")) {
              ...
              if (dc_nodes.IsArray()) {
                for (rapidjson::SizeType i = 0; i < dc_nodes.Size(); ++i) {
                  ...
                  nodes.insert(std::lower_bound(nodes.begin(), nodes.end(), node), node);
                }
              } else {
      ...
      bool operator<(const DataCenter& rhs) const { return id < rhs.id; }
    };
      ...
      if (cluster) {
        if (cluster->IsObject()) {
          if (cluster->HasMember("name") && cluster->HasMember("id") &&
              cluster->HasMember("active_connections") && cluster->HasMember("data_centers")) {
            ...
            if (dcs.IsArray()) {
              for (rapidjson::SizeType i = 0; i < dcs.Size(); ++i) {
                DataCenter dc(dcs[i]);
                data_centers.insert(std::lower_bound(data_centers.begin(), data_centers.end(), dc),
                                    dc);
              }
            } else {
   ...
   */
  void
  create_cluster(const std::vector<unsigned int>& data_center_nodes = DEFAULT_DATA_CENTER_NODES,
   ...
   *                   (default: false)
   */
  void create_cluster(unsigned int data_center_one_nodes, unsigned int data_center_two_nodes = 0,
                      bool with_vnodes = false);
...
private:
   ...
   */
  static void handle_exit(uv_process_t* process, int64_t error_code, int term_signal);
   ...
   */
  static void handle_read(uv_stream_t* stream, ssize_t buffer_length, const uv_buf_t* buffer);
   ...
   */
  const std::string send_post(const std::string& endpoint, const std::string& content = "");
   ...
   */
  Response send_request(Request::Method method, const std::string& endpoint,
