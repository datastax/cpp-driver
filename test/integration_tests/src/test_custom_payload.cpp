...
BOOST_AUTO_TEST_CASE(simple) {
  ...
  if ((version.major_version >= 2 && version.minor_version >= 2) || version.major_version >= 3) {
    ...
    ccm->start_cluster("-Dcassandra.custom_query_handler_class=org.apache.cassandra.cql3."
    ...
    for (PayloadItemMap::const_iterator i = items.begin(); i != items.end(); ++i) {
      cass_custom_payload_set(custom_payload.get(), i->first.c_str(),
                              reinterpret_cast<const cass_byte_t*>(i->second.data()),
                              i->second.size());
    }
    ...
    for (size_t i = 0; i < item_count; ++i) {
      ...
      size_t value_size;
      BOOST_REQUIRE(cass_future_custom_payload_item(future.get(), i, &name, &name_length, &value,
                                                    &value_size) == CASS_OK);
    ...
    std::cout << "Unsupported Test for Cassandra v" << version.to_string()
              << ": Skipping custom_payload/simple" << std::endl;
    BOOST_REQUIRE(true);
  }
}
