...
BOOST_AUTO_TEST_CASE(test_bound)
{
  ...
  test_utils::CassFuturePtr insert_future(cass_session_execute_batch(session.get(), batch));
  ...
  for (int y=0; y<4; y++)
  {
    ...
    test_utils::CassFuturePtr select_future(cass_session_execute(session.get(), select_statement.get()));
