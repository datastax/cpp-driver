...
BOOST_AUTO_TEST_CASE(prepared)
{
  ...
  if (version.major != 1) {
    BatchTests tester;
    test_utils::CassBatchPtr batch(cass_batch_new(tester.session, CASS_BATCH_TYPE_LOGGED));
...
BOOST_AUTO_TEST_CASE(simple)
{
  ...
  if (version.major != 1) {
    ...
    for (int x = 0; x < 4; x++)
    {
      test_utils::CassStatementPtr insert_statement(cass_statement_new(tester.session, insert_query.c_str(), 2));
      BOOST_REQUIRE(cass_statement_bind_int32(insert_statement.get(), 0, x) == CASS_OK);
      BOOST_REQUIRE(cass_statement_bind_string(insert_statement.get(), 1, str(boost::format("test data %s") % x).c_str()) == CASS_OK);
      cass_batch_add_statement(batch.get(), insert_statement.get());
    }
  ...
  if (version.major != 1) {
    ...
    for (int x = 0; x < 1000; x++)
    {
      ...
      if (x % 2 == 0) {
        ...
        insert_statement = test_utils::CassStatementPtr(cass_statement_new(tester.session, insert_query.c_str(), 2));
      }
      BOOST_REQUIRE(cass_statement_bind_int32(insert_statement.get(), 0, x) == CASS_OK);
      BOOST_REQUIRE(cass_statement_bind_string(insert_statement.get(), 1, str(boost::format("test data %s") % x).c_str()) == CASS_OK);
      cass_batch_add_statement(batch.get(), insert_statement.get());
...
BOOST_AUTO_TEST_CASE(invalid_batch_type)
{
  ...
  if (version.major != 1) {
    ...
    test_utils::CassStatementPtr update_statement(cass_statement_new(tester.session, update_query.c_str(), 2));
...
BOOST_AUTO_TEST_CASE(counter_mixed)
{
  ...
  if (version.major != 1) {
    BatchTests tester;
    test_utils::CassBatchPtr batch(cass_batch_new(tester.session, CASS_BATCH_TYPE_COUNTER));
    std::string update_query = str(boost::format("UPDATE %s SET test_val = test_val + ? WHERE tweet_id = ?;") % BatchTests::COUNTER_TABLE_NAME);
    ...
    for (int x = 0; x < 1000; x++)
    {
      ...
      if (x % 2 == 0) {
        ...
        update_statement = test_utils::CassStatementPtr(cass_statement_new(tester.session, update_query.c_str(), 2));
      }
