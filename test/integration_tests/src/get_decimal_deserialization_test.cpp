...
void generate_random_blob( std::vector< cql::cql_byte_t > & output, int size )
{
	output.resize( size );
...
std::string convert_blob_vector_to_string( std::vector< cql::cql_byte_t > const & v )
{	
	std::string res;
...
std::string generate_random_inet_v6()
{	
	std::string res;
	for( int i = 0; i < 32; ++i )	
	{
...
std::string generate_random_inet()
{		
	int arr[4];
	for( int i = 0; i < 4; ++i )
		arr[ i ] = rand() % 256;
...
cql::cql_bigint_t generate_random_time_stamp_2()
{							
...
void generate_random_uuid_2( std::vector< cql::cql_byte_t > & v )
{		
	v.resize( 16 );
...
void convert_timestamp_to_uuid_2( cql::cql_bigint_t ts, std::vector< cql::cql_byte_t > & v_bytes )
{			
	v_bytes.resize( 16 );	
...
std::string make_conversion_uuid_to_string_2( std::vector< cql::cql_byte_t > const & v )
{
	std::string result;
...
{								
	int const number_of_records_inserted_to_one_table = 800;		// number of rows inserted to each table.	
...
//    CQL_CONSISTENCY_EACH_QUORUM  = 0x0007,       wrong  wrong  wrong  wrong  wrong  wrong
	...
	{  //// 1. Check all types in one huge table 
		...
		{					
			...
			if( result->get_uuid( 11, t_11_ ) )
			{
				if( t_11_map[ t_00_ ] != t_11_ )
				{
