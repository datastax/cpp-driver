...
Pool::PoolHandler::PoolHandler(Pool* pool, Connection* connection, RequestHandler* request_handler)
  : pool_(pool)
...
Pool::Pool(const Host& host, uv_loop_t* loop, SSLContext* ssl_context, Logger* logger, const Config& config)
...
Connection* Pool::find_least_busy() {
  ConnectionCollection::iterator it =
      std::max_element(connections_.begin(), connections_.end(), least_busy_comp);
