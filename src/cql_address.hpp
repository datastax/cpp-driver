...
#ifndef __ADDRESS_HPP_INCLUDED__
#define __ADDRESS_HPP_INCLUDED__
...
class Address {
  ...
  public:
    ...
    static bool from_string(const std::string& ip, int port, Address* output = nullptr) {
    ...
    int compare(const Address& a) const {
      ...
      if(family() == AF_INET) {
        ...
        return memcmp(&(addr_in6()->sin6_addr),
  ...
  struct hash<Address> {
    ...
    size_t operator()(const Address& a) const {
      if(a.family() == AF_INET) {
        ...
        return hash_func(std::string(reinterpret_cast<const char*>(&(a.addr_in6()->sin6_addr)),
