...
SET "DOWNLOAD_URL_WINDOWS_INSTALLER_XML=http://wixtoolset.org/releases/"
SET CMAKE=cmake.exe
...
SET BUILD_PACKAGE_PREFIX=dse-cpp-driver
...
SET CASSANDRA_CPP_DRIVER_DIRECTORY=cpp-driver
...
SET OPENSSL_1_0_BRANCH_TAG_VERSION=OpenSSL_1_0_2k
SET OPENSSL_1_1_BRANCH_TAG_VERSION=OpenSSL_1_1_0e
...
SET "ABSOLUTE_DEPENDENCIES_CASSANDRA_CPP_DRIVER_SOURCE_DIRECTORY=%ABSOLUTE_BATCH_DIRECTORY%\%CASSANDRA_CPP_DRIVER_DIRECTORY%"
...
IF !BUILD_DEPENDENCIES_ONLY! EQU !FALSE! (
  ...
  SET "KERBEROS_SDK_RUNTIME_DIRECTORY=!KERBEROS_SDK_DIRECTORY!\!LIBRARY_RUNTIME_DIRECTORY!"
...
IF "!ENABLE_BUILD_PACKAGES!" == "!FALSE!" (
  ...
  IF !BUILD_DEPENDENCIES_ONLY! EQU !FALSE! (
    ...
    IF !DRIVER_GENERATE_SOLUTION! EQU !TRUE! (
      ...
      REM Determine if Kerberos runtime libraries should be copied
      ...
      IF !RUNTIME_LIBRARIES_REQUIRED! EQU !TRUE! (
        ...
        XCOPY /Y /E "!KERBEROS_SDK_RUNTIME_DIRECTORY!\*!TARGET_ARCHITECTURE!.dll" "!DRIVER_RUNTIME_DIRECTORY!" >> "!LOG_DRIVER_BUILD!" 2>&1
        IF NOT !ERRORLEVEL! EQU 0 (
          ECHO FAILED!
          ECHO 	See !LOG_DRIVER_BUILD! for more details
          EXIT /B !EXIT_CODE_BUILD_DRIVER_FAILED!
      ...
      IF !ENABLE_EXAMPLES! EQU !TRUE! (
        ...
        FOR /D %%A IN ("!EXAMPLES_DIRECTORY!\*") DO (
          ...
          XCOPY /Y /E "!KERBEROS_SDK_RUNTIME_DIRECTORY!\*!TARGET_ARCHITECTURE!.dll" "!EXAMPLE_DIRECTORY!" >> "!LOG_DRIVER_BUILD!" 2>&1
          IF NOT !ERRORLEVEL! EQU 0 (
            ECHO FAILED!
            ECHO 	See !LOG_DRIVER_BUILD! for more details
            EXIT /B !EXIT_CODE_BUILD_DRIVER_FAILED!
...
:BUILDDRIVER [source-directory] [build-directory] [install-directory] [libuv-library-directory] [openssl-library-directory] [zlib-library-directory] [libssh2-library-directory] [build-type] [target-architecture] [library-type] [visual-studio-version] [build-examples] [build-integration-tests] [build-unit-tests] [generate-solution] [boost-atomic-directory] [log-filename]
  ...
  SET "DRIVER_CMAKE_COMMAND_LINE=!DRIVER_CMAKE_COMMAND_LINE! -DCASS_MULTICORE_COMPILATION=ON -DCMAKE_INSTALL_PREFIX=^"!DRIVER_INSTALL_DIRECTORY!^" -DCMAKE_BUILD_TYPE=!DRIVER_BUILD_TYPE! -DDSE_BUILD_EXAMPLES=!DRIVER_BUILD_EXAMPLES! -DLIBUV_ROOT_DIR=^"!DRIVER_LIBUV_LIBRARY_DIRECTORY!^" -DDSE_USE_OPENSSL=ON -DOPENSSL_ROOT_DIR=^"!DRIVER_OPENSSL_LIBRARY_DIRECTORY!^""
  IF NOT "!DRIVER_ZLIB_LIBRARY_DIRECTORY!" == "" (
    SET "DRIVER_CMAKE_COMMAND_LINE=!DRIVER_CMAKE_COMMAND_LINE! -DDSE_USE_ZLIB=ON -DZLIB_ROOT_DIR=^"!DRIVER_ZLIB_LIBRARY_DIRECTORY!^""
  ) ELSE (
    SET "DRIVER_CMAKE_COMMAND_LINE=!DRIVER_CMAKE_COMMAND_LINE! -DDSE_USE_ZLIB=OFF"
  )
  IF "!DRIVER_LIBRARY_TYPE!" == "!LIBRARY_TYPE_SHARED!" (
    SET "DRIVER_CMAKE_COMMAND_LINE=!DRIVER_CMAKE_COMMAND_LINE! -DDSE_BUILD_STATIC=OFF"
  ) ELSE (
    SET "DRIVER_CMAKE_COMMAND_LINE=!DRIVER_CMAKE_COMMAND_LINE! -DDSE_BUILD_STATIC=ON -DDSE_USE_STATIC_LIBS=ON"
  )
  IF !DRIVER_BUILD_TESTS! EQU !TRUE! SET "DRIVER_CMAKE_COMMAND_LINE=!DRIVER_CMAKE_COMMAND_LINE! -DDSE_BUILD_TESTS=ON"
  IF !DRIVER_BUILD_INTEGRATION_TESTS! EQU !TRUE! SET "DRIVER_CMAKE_COMMAND_LINE=!DRIVER_CMAKE_COMMAND_LINE! -DDSE_BUILD_INTEGRATION_TESTS=ON"
  IF !DRIVER_BUILD_UNIT_TESTS! EQU !TRUE! SET "DRIVER_CMAKE_COMMAND_LINE=!DRIVER_CMAKE_COMMAND_LINE! -DDSE_BUILD_UNIT_TESTS=ON"
  IF NOT "!TESTS_LIBSSH2_LIBRARY_DIRECTORY!" == "" (
    SET "DRIVER_CMAKE_COMMAND_LINE=!DRIVER_CMAKE_COMMAND_LINE! -DDSE_USE_LIBSSH2=ON -DLIBSSH2_ROOT_DIR=^"!TESTS_LIBSSH2_LIBRARY_DIRECTORY!^""
  ) ELSE (
    SET "DRIVER_CMAKE_COMMAND_LINE=!DRIVER_CMAKE_COMMAND_LINE! -DDSE_LIBSSH2=OFF"
  )
  IF NOT "!DRIVER_BOOST_DEPENDENCY_SOURCE_DIRECTORY!" == "" (
    SET "DRIVER_CMAKE_COMMAND_LINE=!DRIVER_CMAKE_COMMAND_LINE! -DDSE_USE_BOOST_ATOMIC=ON"
