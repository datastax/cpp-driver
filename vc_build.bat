...
SET ARGUMENT_BUILD_TYPE_RELEASE_WITH_DEBUG_INFO=--RELEASE-WITH-DEBUG-INFO
...
SET BUILD_TYPE_RELEASE_WITH_DEBUG_INFO=RELWITHDEBINFO
...
SET OPENSSL_1_0_BRANCH_TAG_VERSION=OpenSSL_1_0_2n
SET OPENSSL_1_1_BRANCH_TAG_VERSION=OpenSSL_1_1_0g
SET OPENSSL_PACKAGE_VERSION=1.0.2n
...
SET BUILD_TYPE=%BUILD_TYPE_RELEASE_WITH_DEBUG_INFO%
...
IF NOT [%1] == [] (
  ...
  REM Build type (debug/release/release with debug info)
  ...
  IF "!ARGUMENT!" == "!ARGUMENT_BUILD_TYPE_RELEASE_WITH_DEBUG_INFO!" (
    SET BUILD_TYPE=!BUILD_TYPE_RELEASE_WITH_DEBUG_INFO!
  )
...
REM Ensure packages are disabled (if tests)
...
IF "!ENABLE_BUILD_PACKAGES!" == "!FALSE!" (
  ...
  IF !ENABLE_ZLIB! EQU !TRUE! (
    IF NOT EXIST "!ABSOLUTE_ZLIB_LIBRARY_DIRECTORY!" (
      CALL :BUILDZLIB "!ABSOLUTE_DEPENDENCIES_DIRECTORY!\!DEPENDENCIES_SOURCE_DIRECTORY!\!ZLIB_DIRECTORY!" "!ABSOLUTE_ZLIB_LIBRARY_DIRECTORY!" !TARGET_ARCHITECTURE! !VISUAL_STUDIO_INTERNAL_SHORTHAND_VERSION! "!LOG_ZLIB_BUILD!"
      IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
    )
  ) ELSE (
  ...
  IF !BUILD_DEPENDENCIES_ONLY! EQU !FALSE! (
    ...
    IF !DRIVER_GENERATE_SOLUTION! EQU !TRUE! (
      ...
      IF !RUNTIME_LIBRARIES_REQUIRED! EQU !TRUE! (
        ...
        COPY /Y "!DRIVER_BUILD_DIRECTORY!\*.jar" "!DRIVER_RUNTIME_DIRECTORY!" >> "!LOG_DRIVER_BUILD!" 2>&1
        IF NOT !ERRORLEVEL! EQU 0 (
          ECHO FAILED!
          ECHO 	See !LOG_DRIVER_BUILD! for more details
          EXIT /B !EXIT_CODE_BUILD_DRIVER_FAILED!
...
:DISPLAYHELP
  ...
  ECHO     !ARGUMENT_BUILD_TYPE_RELEASE!                         Enable release build
  ECHO     !ARGUMENT_BUILD_TYPE_RELEASE_WITH_DEBUG_INFO!         Enable release w/ dbginfo build ^(default^)
...
:BUILDLIBUV [source-directory] [install-directory] [target-architecture] [library-type] [visual-studio-version] [is-clean-after-install] [log-filename]
  ...
  IF "!LIBUV_LIBRARY_TYPE!" == "static" (
    IF EXIST Release\obj\libuv\vc*.pdb (
      COPY /Y Release\obj\libuv\vc*.pdb "!LIBUV_INSTALL_DIRECTORY!\!LIBRARY_BINARY_DIRECTORY!" >> "!LIBUV_LOG_FILENAME!" 2>&1
      IF NOT !ERRORLEVEL! EQU 0 (
        ECHO FAILED!
      ...
      IF EXIST Release\obj\libuv\libuv.pdb (
        COPY /Y Release\obj\libuv\libuv.pdb "!LIBUV_INSTALL_DIRECTORY!\!LIBRARY_BINARY_DIRECTORY!" >> "!LIBUV_LOG_FILENAME!" 2>&1
        IF NOT !ERRORLEVEL! EQU 0 (
          ECHO FAILED!
          ECHO 	See !OPENSSL_LOG_FILENAME! for more details
          EXIT /B !EXIT_CODE_BUILD_DEPENDENCY_FAILED!
...
:BUILDOPENSSL [source-directory] [install-directory] [zlib-library-directory] [target-architecture] [library-type] [is-force-shared] [openssl-version] [is-clean-after-install] [log-filename]
  ...
  IF NOT "!DEPENDENCY_ZLIB_LIBRARY_DIRECTORY!" == "" (
    ...
    SET ZLIB_CONFIGURE_OPTION=zlib-dynamic
    IF "!OPENSSL_LIBRARY_TYPE!" == "!LIBRARY_TYPE_STATIC!" (
      SET DEPENDENCY_ZLIB_LIBRARY_FILENAME=zlibstatic.lib
      SET ZLIB_CONFIGURE_OPTION=zlib
    )
    ...
    SET "OPENSSL_CONFIGURE_OPTIONS=!OPENSSL_CONFIGURE_OPTIONS! !ZLIB_CONFIGURE_OPTION! --with-zlib-include=^"!SHORTENED_DEPENDENCY_ZLIB_LIBRARY_DIRECTORY!\!LIBRARY_INCLUDE_DIRECTORY!^" --with-zlib-lib=^"!SHORTENED_DEPENDENCY_ZLIB_LIBRARY_DIRECTORY!\!LIBRARY_BINARY_DIRECTORY!\!DEPENDENCY_ZLIB_LIBRARY_FILENAME!^""
  )
  ...
  IF EXIST tmp32\lib.pdb (
    COPY /Y tmp32\lib.pdb "!SHORTENED_OPENSSL_INSTALL_DIRECTORY!\!LIBRARY_BINARY_DIRECTORY!" >> "!OPENSSL_LOG_FILENAME!" 2>&1
    IF NOT !ERRORLEVEL! EQU 0 (
      ECHO FAILED!
      ECHO 	See !OPENSSL_LOG_FILENAME! for more details
      EXIT /B !EXIT_CODE_BUILD_DEPENDENCY_FAILED!
  ...
  IF EXIST tmp32dll\lib.pdb (
    COPY /Y tmp32dll\lib.pdb "!SHORTENED_OPENSSL_INSTALL_DIRECTORY!\!LIBRARY_BINARY_DIRECTORY!" >> "!OPENSSL_LOG_FILENAME!" 2>&1
    IF NOT !ERRORLEVEL! EQU 0 (
      ECHO FAILED!
      ECHO 	See !OPENSSL_LOG_FILENAME! for more details
      EXIT /B !EXIT_CODE_BUILD_DEPENDENCY_FAILED!
...
REM @param visual-studio-version Shortened Visual Studio version
REM @param log-filename Absolute path and filename for log output
:BUILDZLIB [source-directory] [install-directory] [target-architecture] [visual-studio-version] [log-filename]
  REM Create library variables from arguments
  SET "ZLIB_SOURCE_DIRECTORY=%~1"
  SHIFT
  SET "ZLIB_INSTALL_DIRECTORY=%~1"
  SHIFT
  SET "ZLIB_TARGET_ARCHITECTURE=%~1"
  SHIFT
  SET "ZLIB_VISUAL_STUDIO_VERSION=%~1"
  SHIFT
  ...
  SET "ZLIB_CMAKE_COMMAND_LINE=-G ^"Visual Studio !ZLIB_VISUAL_STUDIO_VERSION!"
  IF !ZLIB_TARGET_ARCHITECTURE! EQU !ARCHITECTURE_64BIT! (
    SET "ZLIB_CMAKE_COMMAND_LINE=!ZLIB_CMAKE_COMMAND_LINE! Win64"
  )
  SET "ZLIB_CMAKE_COMMAND_LINE=!ZLIB_CMAKE_COMMAND_LINE!^""
  SET "ZLIB_CMAKE_COMMAND_LINE=!ZLIB_CMAKE_COMMAND_LINE! -DCMAKE_INSTALL_PREFIX="!ZLIB_INSTALL_DIRECTORY!" -DCMAKE_BUILD_TYPE=!BUILD_TYPE_RELEASE_WITH_DEBUG_INFO! .."
  ECHO !CMAKE! !ZLIB_CMAKE_COMMAND_LINE! "!ZLIB_SOURCE_DIRECTORY!" >> "!ZLIB_LOG_FILENAME!" 2>&1
  !CMAKE! !CMAKE_OPTIONS! !ZLIB_CMAKE_COMMAND_LINE! "!ZLIB_SOURCE_DIRECTORY!" >> "!ZLIB_LOG_FILENAME!" 2>&1
  IF NOT !ERRORLEVEL! EQU 0 (
    ECHO FAILED!
  ...
  ECHO done.
  IF "!LIBRARY_TYPE!" == "!LIBRARY_TYPE_STATIC!" (
    !PERL! -p -i".backup" -e "s/\/MD/\/MT/g" CMakeCache.txt
  )
  ECHO | SET /P="Building zlib ... "
  !CMAKE! !CMAKE_OPTIONS! --build . --config !BUILD_TYPE_RELEASE_WITH_DEBUG_INFO! >> "!ZLIB_LOG_FILENAME!" 2>&1
  IF NOT !ERRORLEVEL! EQU 0 (
    ECHO FAILED!
    ECHO 	See!ZLIB_LOG_FILENAME! for more details
    EXIT /B !EXIT_CODE_BUILD_DEPENDENCY_FAILED!
  ...
  !CMAKE! !CMAKE_OPTIONS! --build . --config !BUILD_TYPE_RELEASE_WITH_DEBUG_INFO! --target install >> "!ZLIB_LOG_FILENAME!" 2>&1
  IF NOT !ERRORLEVEL! EQU 0 (
    ECHO FAILED!
    ECHO 	See !ZLIB_LOG_FILENAME! for more details
    EXIT /B !EXIT_CODE_BUILD_DEPENDENCY_FAILED!
  )
  IF EXIST zlib.dir\!BUILD_TYPE_RELEASE_WITH_DEBUG_INFO!\vc*.pdb (
    COPY /Y zlib.dir\!BUILD_TYPE_RELEASE_WITH_DEBUG_INFO!\vc*.pdb "!ZLIB_INSTALL_DIRECTORY!\!LIBRARY_RUNTIME_DIRECTORY!" >> "!ZLIB_LOG_FILENAME!" 2>&1
    IF NOT !ERRORLEVEL! EQU 0 (
      ECHO FAILED!
      ECHO 	See !ZLIB_LOG_FILENAME! for more details
      EXIT /B !EXIT_CODE_BUILD_DEPENDENCY_FAILED!
    ...
    IF EXIST zlib.dir\!BUILD_TYPE_RELEASE_WITH_DEBUG_INFO!\zlib.pdb (
      COPY /Y zlib.dir\!BUILD_TYPE_RELEASE_WITH_DEBUG_INFO!\zlib.pdb "!ZLIB_INSTALL_DIRECTORY!\!LIBRARY_RUNTIME_DIRECTORY!" >> "!ZLIB_LOG_FILENAME!" 2>&1
      IF NOT !ERRORLEVEL! EQU 0 (
        ECHO FAILED!
        ECHO 	See !ZLIB_LOG_FILENAME! for more details
        EXIT /B !EXIT_CODE_BUILD_DEPENDENCY_FAILED!
  ...
  IF EXIST zlibstatic.dir\!BUILD_TYPE_RELEASE_WITH_DEBUG_INFO!\vc*.pdb (
    COPY /Y zlibstatic.dir\!BUILD_TYPE_RELEASE_WITH_DEBUG_INFO!\vc*.pdb "!ZLIB_INSTALL_DIRECTORY!\!LIBRARY_BINARY_DIRECTORY!" >> "!ZLIB_LOG_FILENAME!" 2>&1
    IF NOT !ERRORLEVEL! EQU 0 (
      ECHO FAILED!
      ECHO 	See !ZLIB_LOG_FILENAME! for more details
      EXIT /B !EXIT_CODE_BUILD_DEPENDENCY_FAILED!
    ...
    IF EXIST zlibstatic.dir\!BUILD_TYPE_RELEASE_WITH_DEBUG_INFO!\zlibstatic.pdb (
      COPY /Y zlibstatic.dir\!BUILD_TYPE_RELEASE_WITH_DEBUG_INFO!\zlibstatic.pdb "!ZLIB_INSTALL_DIRECTORY!\!LIBRARY_BINARY_DIRECTORY!" >> "!ZLIB_LOG_FILENAME!" 2>&1
      IF NOT !ERRORLEVEL! EQU 0 (
        ECHO FAILED!
        ECHO 	See !ZLIB_LOG_FILENAME! for more details
        EXIT /B !EXIT_CODE_BUILD_DEPENDENCY_FAILED!
...
:BUILDLIBSSH2 [source-directory] [install-directory] [openssl-library-directory] [zlib-library-directory] [target-architecture] [visual-studio-version] [is-clean-after-install] [log-filename]
  ...
  SET "LIBSSH2_CMAKE_COMMAND_LINE=!LIBSSH2_CMAKE_COMMAND_LINE! -DCMAKE_BUILD_TYPE=!BUILD_TYPE_RELEASE_WITH_DEBUG_INFO! -DCMAKE_INSTALL_PREFIX=^"!LIBSSH2_INSTALL_DIRECTORY!^" -DBUILD_SHARED_LIBS=OFF -DBUILD_EXAMPLES=OFF -DBUILD_TESTING=OFF -DOPENSSL_ROOT_DIR=^"!DEPENDENCY_OPENSSL_LIBRARY_DIRECTORY!^""
  IF NOT "!DEPENDENCY_ZLIB_LIBRARY_DIRECTORY!" == "" (
  ...
  !CMAKE! !CMAKE_OPTIONS! --build . --config !BUILD_TYPE_RELEASE_WITH_DEBUG_INFO! >> "!LIBSSH2_LOG_FILENAME!" 2>&1
  IF NOT !ERRORLEVEL! EQU 0 (
    ECHO FAILED!
    ECHO 	See !LIBSSH2_LOG_FILENAME! for more details
    EXIT /B !EXIT_CODE_BUILD_DEPENDENCY_FAILED!
  ...
  !CMAKE! !CMAKE_OPTIONS! --build . --config !BUILD_TYPE_RELEASE_WITH_DEBUG_INFO! --target install >> "!LIBSSH2_LOG_FILENAME!" 2>&1
  IF NOT !ERRORLEVEL! EQU 0 (
    ECHO FAILED!
    ECHO 	See !LIBSSH2_LOG_FILENAME! for more details
    EXIT /B !EXIT_CODE_BUILD_DEPENDENCY_FAILED!
  )
  IF EXIST src\libssh2.dir\!BUILD_TYPE_RELEASE_WITH_DEBUG_INFO!\vc*.pdb (
    COPY /Y src\libssh2.dir\!BUILD_TYPE_RELEASE_WITH_DEBUG_INFO!\vc*.pdb "!LIBSSH2_INSTALL_DIRECTORY!\!LIBRARY_BINARY_DIRECTORY!" >> "!LIBSSH2_LOG_FILENAME!" 2>&1
    IF NOT !ERRORLEVEL! EQU 0 (
      ECHO FAILED!
      ECHO 	See !LIBSSH2_LOG_FILENAME! for more details
      EXIT /B !EXIT_CODE_BUILD_DEPENDENCY_FAILED!
    ...
    IF EXIST src\libssh2.dir\!BUILD_TYPE_RELEASE_WITH_DEBUG_INFO!\libssh2.pdb (
      COPY /Y src\libssh2.dir\!BUILD_TYPE_RELEASE_WITH_DEBUG_INFO!\libssh2.pdb "!LIBSSH2_INSTALL_DIRECTORY!\!LIBRARY_BINARY_DIRECTORY!" >> "!LIBSSH2_LOG_FILENAME!" 2>&1
      IF NOT !ERRORLEVEL! EQU 0 (
        ECHO FAILED!
        ECHO 	See !LIBSSH2_LOG_FILENAME! for more details
        EXIT /B !EXIT_CODE_BUILD_DEPENDENCY_FAILED!
