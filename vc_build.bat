...
SET "OPENSSL_PACKAGE_VERSION=1.0.1l"
...
IF "!ENABLE_BUILD_PACKAGES!" == "!FALSE!" (
	...
	IF !ENABLE_OPENSSL! EQU !TRUE! (
		...
		IF NOT EXIST "!ABSOLUTE_OPENSSL_LIBRARY_DIRECTORY!" (
			CALL :BUILDOPENSSL "!ABSOLUTE_DEPENDENCIES_DIRECTORY!\!DEPENDENCIES_SOURCE_DIRECTORY!\!OPENSSL_DIRECTORY!" "!ABSOLUTE_OPENSSL_LIBRARY_DIRECTORY!" !TARGET_ARCHITECTURE! !LIBRARY_TYPE! !FALSE! "!LOG_OPENSSL_BUILD!"
			IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
		)
	)
	...
	IF NOT EXIST "!ABSOLUTE_DRIVER_LIBRARY_DIRECTORY!" (
		CALL :BUILDDRIVER "!ABSOLUTE_BATCH_DIRECTORY!" "!ABSOLUTE_BUILD_DIRECTORY!\!DRIVER_DRIVER_DIRECTORY!" "!ABSOLUTE_DRIVER_LIBRARY_DIRECTORY!" "!ABSOLUTE_LIBUV_LIBRARY_DIRECTORY!" "!ABSOLUTE_OPENSSL_LIBRARY_DIRECTORY!" !BUILD_TYPE! !TARGET_ARCHITECTURE! !LIBRARY_TYPE! !VISUAL_STUDIO_INTERNAL_SHORTHAND_VERSION! "!LOG_DRIVER_BUILD!"
		IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
	)
	...
	FOR %%A IN (!AVAILABLE_VISUAL_STUDIO_VERSIONS!) DO (
		...
		FOR %%C IN (!AVAILABLE_TARGET_ARCHITECTURES!) DO (
			...
			ECHO !ZIP! a -tzip !ABSOLUTE_PACKAGES_DIRECTORY!\datastax-cpp-driver-!BUILD_PACKAGE_VERSION!-win%%C-msvc!VISUAL_STUDIO_INTERNAL_VERSION!.zip -r !ABSOLUTE_DRIVER_PACKAGE_INSTALLATION_DIRECTORY!\ -xr^^!!DEPENDENCIES_DIRECTORY! >> "!LOG_PACKAGE_BUILD!"
			!ZIP! a -tzip "!ABSOLUTE_PACKAGES_DIRECTORY!\datastax-cpp-driver-!BUILD_PACKAGE_VERSION!-win%%C-msvc!VISUAL_STUDIO_INTERNAL_VERSION!.zip" -r "!ABSOLUTE_DRIVER_PACKAGE_INSTALLATION_DIRECTORY!\*" -xr^^!!DEPENDENCIES_DIRECTORY! >> "!LOG_PACKAGE_BUILD!" 2>&1
...
REM                               install; false otherwise
REM @param log-filename Absolute path and filename for log output
:BUILDOPENSSL [source-directory] [install-directory] [target-architecture] [library-type] [is-clean-after-install] [log-filename]
	...
	CALL :SHORTENPATH "%~2" SHORTENED_OPENSSL_INSTALL_DIRECTORY
	ECHO !PERL! !OPENSSL_CONFIGURE_OPTIONS! enable-static-engine --openssldir=!SHORTENED_OPENSSL_INSTALL_DIRECTORY! --prefix=!SHORTENED_OPENSSL_INSTALL_DIRECTORY! >> "%~6"
	!PERL! !OPENSSL_CONFIGURE_OPTIONS! enable-static-engine --openssldir=!SHORTENED_OPENSSL_INSTALL_DIRECTORY! --prefix=!SHORTENED_OPENSSL_INSTALL_DIRECTORY! >> "%~6" 2>&1
	IF NOT !ERRORLEVEL! EQU 0 (
		ECHO FAILED!
		ECHO 	See %~6 for more details
		EXIT /B !EXIT_CODE_BUILD_DEPENDENCY_FAILED!
	)
	ECHO done.
	ECHO | SET /P=Building OpenSSL ... 
	IF %~3 EQU !ARCHITECTURE_32BIT! (
		CALL ms\do_ms.bat >> "%~6" 2>&1
	) ELSE (
		CALL ms\do_win64a.bat >> "%~6" 2>&1
	)
	IF "%~4" == "!LIBRARY_TYPE_SHARED!" (
	...
	!NMAKE! /F !OPENSSL_MAKEFILE! >> "%~6" 2>&1
	IF NOT !ERRORLEVEL! EQU 0 (
		ECHO FAILED!
		ECHO 	See %~6 for more details
		EXIT /B !EXIT_CODE_BUILD_DEPENDENCY_FAILED!
	...
	!NMAKE! /F !OPENSSL_MAKEFILE! install >> "%~6" 2>&1
	IF NOT !ERRORLEVEL! EQU 0 (
		ECHO FAILED!
		ECHO 	See %~6 for more details
		EXIT /B !EXIT_CODE_BUILD_DEPENDENCY_FAILED!
	...
	IF "%~5" == "!TRUE!" (
		...
		!NMAKE! /F !OPENSSL_MAKEFILE! clean >> "%~6" 2>&1
		ECHO done.
	)
	POPD
	EXIT /B
...
REM @param log-filename Absolute path and filename for log output
:BUILDDRIVER [source-directory] [build-directory] [install-directory] [libuv-library-directory] [openssl-library-directory] [build-type] [target-architecture] [library-type] [visual-studio-version] [log-filename]
	...
	ECHO 	!CMAKE! !DRIVER_CMAKE_COMMAND_LINE! "!DRIVER_SOURCE_DIRECTORY!" >> "!DRIVER_LOG_FILENAME!"
	...
	IF NOT "!DRIVER_VISUAL_STUDIO_VERSION!" == "" (
		ECHO !MSBUILD! cassandra.sln /T:!DRIVER_VISUAL_STUDIO_PROJECT! /P:Configuration=!DRIVER_BUILD_TYPE! /P:Platform=!VISUAL_STUDIO_PLATFORM_TYPE! /CLP:NoSummary;NoItemAndPropertyList;Verbosity=minimal /NOLOGO >> "!DRIVER_LOG_FILENAME!"
		!MSBUILD! cassandra.sln /T:!DRIVER_VISUAL_STUDIO_PROJECT! /P:Configuration=!DRIVER_BUILD_TYPE! /P:Platform=!VISUAL_STUDIO_PLATFORM_TYPE! /CLP:NoSummary;NoItemAndPropertyList;Verbosity=minimal /NOLOGO >> "!DRIVER_LOG_FILENAME!" 2>&1
	) ELSE (
		ECHO !NMAKE! /F Makefile >> "!DRIVER_LOG_FILENAME!"
		!NMAKE! /F Makefile >> "!DRIVER_LOG_FILENAME!" 2>&1
	)
	IF NOT !ERRORLEVEL! EQU 0 (
		ECHO FAILED!
		ECHO 	See !DRIVER_LOG_FILENAME! for more details
	...
	IF !DEVENV_FOUND! EQU !TRUE! (
		...
		ECHO !DEVENV! cassandra.sln /BUILD "!DRIVER_BUILD_TYPE!|!VISUAL_STUDIO_PLATFORM_TYPE!" /PROJECT INSTALL >> "!DRIVER_LOG_FILENAME!"
		!DEVENV! cassandra.sln /BUILD "!DRIVER_BUILD_TYPE!|!VISUAL_STUDIO_PLATFORM_TYPE!" /PROJECT INSTALL >> "!DRIVER_LOG_FILENAME!" 2>&1
	) ELSE (
		IF NOT EXIST "!DRIVER_INSTALL_DIRECTORY!\!LIBRARY_INCLUDE_DIRECTORY!" MKDIR "!DRIVER_INSTALL_DIRECTORY!\!LIBRARY_INCLUDE_DIRECTORY!"
		XCOPY /E /Y "!DRIVER_SOURCE_DIRECTORY!\include" "!DRIVER_INSTALL_DIRECTORY!\!LIBRARY_INCLUDE_DIRECTORY!" >> "!DRIVER_LOG_FILENAME!" 2>&1
		IF NOT !ERRORLEVEL! EQU 0 (
			ECHO FAILED!
			ECHO 	See !DRIVER_LOG_FILENAME! for more details
			EXIT /B !EXIT_CODE_BUILD_DRIVER_FAILED!
		)
		IF NOT EXIST "!DRIVER_INSTALL_DIRECTORY!\!LIBRARY_BINARY_DIRECTORY!" MKDIR "!DRIVER_INSTALL_DIRECTORY!\!LIBRARY_BINARY_DIRECTORY!"
		IF NOT "!DRIVER_VISUAL_STUDIO_VERSION!" == "" (
			XCOPY /Y !DRIVER_BUILD_TYPE!\*.lib "!DRIVER_INSTALL_DIRECTORY!\!LIBRARY_BINARY_DIRECTORY!" >> "!DRIVER_LOG_FILENAME!" 2>&1
		) ELSE (
			XCOPY /Y *.lib "!DRIVER_INSTALL_DIRECTORY!\!LIBRARY_BINARY_DIRECTORY!" >> "!DRIVER_LOG_FILENAME!" 2>&1
		)
		IF NOT !ERRORLEVEL! EQU 0 (
			ECHO FAILED!
			ECHO 	See !DRIVER_LOG_FILENAME! for more details
		...
		IF "!DRIVER_LIBRARY_TYPE!" == "!LIBRARY_TYPE_SHARED!" (
			IF NOT EXIST "!DRIVER_INSTALL_DIRECTORY!\!LIBRARY_RUNTIME_DIRECTORY!" MKDIR "!DRIVER_INSTALL_DIRECTORY!\!LIBRARY_RUNTIME_DIRECTORY!"
			IF NOT "!DRIVER_VISUAL_STUDIO_VERSION!" == "" (
				XCOPY /Y !DRIVER_BUILD_TYPE!\*.dll "!DRIVER_INSTALL_DIRECTORY!\!LIBRARY_RUNTIME_DIRECTORY!" >> "!DRIVER_LOG_FILENAME!" 2>&1
			) ELSE (
				XCOPY /Y *.dll "!DRIVER_INSTALL_DIRECTORY!\!LIBRARY_RUNTIME_DIRECTORY!" >> "!DRIVER_LOG_FILENAME!" 2>&1
			)
		)
	)
	IF NOT !ERRORLEVEL! EQU 0 (
