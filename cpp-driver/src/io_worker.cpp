...
String IOWorker::keyspace() const {
...
bool IOWorker::prepare_all(const Host::Ptr& current_host,
  ...
  PrepareAllHandler::Ptr prepare_all_handler(
        Memory::allocate<PrepareAllHandler>(current_host,
                                            response,
                                            request_handler,
...
void IOWorker::on_execute(uv_async_t* async) {
  ...
  while (remaining != 0 && io_worker->request_queue_.dequeue(temp)) {
    ...
    if (request_handler) {
      ...
      RequestExecution::Ptr request_execution(Memory::allocate<RequestExecution>(request_handler,
