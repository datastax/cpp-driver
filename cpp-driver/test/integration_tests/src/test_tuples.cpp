...
#define NESTED_COLLECTION_SIZE_LIMITER \
  50 // Limit nested collections size (as to not exceed high water mark)
...
private:
   ...
   */
  std::string table_name_builder(CassValueType tuple_type, unsigned int size,
                                 CassValueType collection_type) {
    ...
    if (collection_type == CASS_VALUE_TYPE_LIST || collection_type == CASS_VALUE_TYPE_MAP ||
        collection_type == CASS_VALUE_TYPE_SET || collection_type == CASS_VALUE_TYPE_TUPLE) {
   ...
   */
  std::string tuple_cql_builder(CassValueType tuple_type, unsigned int size,
                                CassValueType collection_type, bool is_frozen = true) {
    std::string cql_value_type = test_utils::get_value_type(tuple_type);
...
public:
   ...
   */
  template <class T>
  void insert_varying_sized_value(CassValueType tuple_type, T tuple_values, unsigned int size = 1,
                                  CassValueType collection_type = CASS_VALUE_TYPE_UNKNOWN) {
    ...
    std::string create_table =
        "CREATE TABLE " + table_name + "(key timeuuid PRIMARY KEY, value " + tuple_cql + ")";
    ...
    for (unsigned int i = 0; i < size; ++i) {
      ...
      if (collection_type == CASS_VALUE_TYPE_LIST || collection_type == CASS_VALUE_TYPE_MAP ||
          collection_type == CASS_VALUE_TYPE_SET || collection_type == CASS_VALUE_TYPE_TUPLE) {
        test_utils::CassCollectionPtr collection(
            cass_collection_new(static_cast<CassCollectionType>(collection_type), size));
        for (unsigned int j = 0; j < size; ++j) {
          BOOST_REQUIRE_EQUAL(test_utils::Value<T>::append(collection.get(), tuple_values),
                              CASS_OK);
          if (collection_type == CASS_VALUE_TYPE_MAP) {
            BOOST_REQUIRE_EQUAL(test_utils::Value<T>::append(collection.get(), tuple_values),
                                CASS_OK);
    ...
    while (cass_iterator_next(tuple_iterator.get())) {
      ...
      if (cass_value_is_collection(abstract_collection_value)) {
        ...
        BOOST_REQUIRE_EQUAL(cass_value_secondary_sub_type(abstract_collection_value),
                            collection_type == CASS_VALUE_TYPE_MAP ? tuple_type
                                                                   : CASS_VALUE_TYPE_UNKNOWN);
        BOOST_REQUIRE_EQUAL(cass_value_item_count(abstract_collection_value), size);

        test_utils::CassIteratorPtr subtype_iterator(
            cass_iterator_from_collection(abstract_collection_value));
        int subtype_count = 0;
        while (cass_iterator_next(subtype_iterator.get())) {
          ++subtype_count;
          T collection_value;
          BOOST_REQUIRE(cass_value_type(cass_iterator_get_value(subtype_iterator.get())) ==
                        tuple_type);
          BOOST_REQUIRE_EQUAL(
              test_utils::Value<T>::get(cass_iterator_get_value(subtype_iterator.get()),
                                        &collection_value),
              CASS_OK);
        ...
        T tuple_value;
        BOOST_REQUIRE_EQUAL(test_utils::Value<T>::get(abstract_collection_value, &tuple_value),
                            CASS_OK);
   ...
   */
  template <class T>
  void insert_varying_sized_null_value(CassValueType tuple_type, unsigned int size = 1,
                                       CassValueType collection_type = CASS_VALUE_TYPE_UNKNOWN) {
    // Create the table for the test
    ...
    while (cass_iterator_next(tuple_iterator.get())) {
      ...
      if (cass_value_is_collection(abstract_collection_value)) {
        ...
        T collection_value;
        BOOST_REQUIRE_EQUAL(test_utils::Value<T>::get(abstract_collection_value, &collection_value),
                            CASS_ERROR_LIB_NULL_VALUE);
      }
    }
    BOOST_REQUIRE_EQUAL(count, size);
...
// TODO: Determine why nested collections are not working properly (values are inserted as seen via
// cqlsh) const CassValueType TupleTests::nested_collection_types_[] = { CASS_VALUE_TYPE_UNKNOWN,
// CASS_VALUE_TYPE_LIST, CASS_VALUE_TYPE_MAP, CASS_VALUE_TYPE_SET, CASS_VALUE_TYPE_TUPLE };
  ...
  if ((version.major_version >= 2 && version.minor_version >= 1) || version.major_version >= 3) {
    ...
    // Full tuples
    {
      ...
      for (unsigned int size = 1; size <= 1; ++size) {
        ...
        test_utils::Value<cass_float_t>::tuple_set(tuple.get(), 2,
                                                   static_cast<cass_float_t>(size * 100.0f));
        ...
        if (size % 2 == 0) {
          test_utils::CassPreparedPtr prepared =
              test_utils::prepare(tester.session, select_query.c_str());
          statement = test_utils::CassStatementPtr(cass_prepared_bind(prepared.get()));
        }
        BOOST_REQUIRE_EQUAL(cass_statement_bind_int32(statement.get(), 0, size), CASS_OK);
        test_utils::CassFuturePtr future =
        ...
        BOOST_REQUIRE_EQUAL(test_utils::Value<cass_int32_t>::get(
                                cass_iterator_get_value(iterator.get()), &value_integer),
                            CASS_OK);
        BOOST_REQUIRE(test_utils::Value<cass_int32_t>::equal(value_integer, size * 10));
        cass_iterator_next(iterator.get());
        CassString value_string;
        BOOST_REQUIRE_EQUAL(test_utils::Value<CassString>::get(
                                cass_iterator_get_value(iterator.get()), &value_string),
                            CASS_OK);
        ...
        BOOST_REQUIRE_EQUAL(test_utils::Value<cass_float_t>::get(
                            ...
                            CASS_OK);
        BOOST_REQUIRE(test_utils::Value<cass_float_t>::equal(
            value_float, static_cast<cass_float_t>(size * 100.0f)));
      }
    }
    ...
    std::cout << "Unsupported Test for Cassandra v" << version.to_string()
              << ": Skipping tuples/read_write" << std::endl;
    BOOST_REQUIRE(true);
  }
}
  ...
  if ((version.major_version >= 2 && version.minor_version >= 1) || version.major_version >= 3) {
    ...
    for (unsigned int i = 0; i < (sizeof(tester.sizes_) / sizeof(int)); ++i) {
      ...
      for (unsigned int j = 0;
           j < (sizeof(tester.nested_collection_types_) / sizeof(CassValueType)); ++j) {
        CassValueType nested_collection_type = tester.nested_collection_types_[j];
        if (nested_collection_type == CASS_VALUE_TYPE_UNKNOWN ||
            size <= NESTED_COLLECTION_SIZE_LIMITER) {
          {
            CassString value("Test Value");
            tester.insert_varying_sized_value<CassString>(CASS_VALUE_TYPE_ASCII, value, size,
                                                          nested_collection_type);
            tester.insert_varying_sized_value<CassString>(
                CASS_VALUE_TYPE_VARCHAR, value, size,
                nested_collection_type); // NOTE: text is alias for varchar
          }

          {
            cass_int64_t value = 1234567890;
            tester.insert_varying_sized_value<cass_int64_t>(CASS_VALUE_TYPE_BIGINT, value, size,
                                                            nested_collection_type);
            tester.insert_varying_sized_value<cass_int64_t>(CASS_VALUE_TYPE_TIMESTAMP, value, size,
                                                            nested_collection_type);
          }

          {
            ...
            tester.insert_varying_sized_value<CassBytes>(CASS_VALUE_TYPE_BLOB, value, size,
                                                         nested_collection_type);
            tester.insert_varying_sized_value<CassBytes>(CASS_VALUE_TYPE_VARINT, value, size,
                                                         nested_collection_type);
          }

          tester.insert_varying_sized_value<cass_bool_t>(CASS_VALUE_TYPE_BOOLEAN, cass_true, size,
                                                         nested_collection_type);

          {
            ...
            tester.insert_varying_sized_value<CassDecimal>(CASS_VALUE_TYPE_DECIMAL, value, size,
                                                           nested_collection_type);
          }
          ...
          if ((version.major_version >= 3 && version.minor_version >= 10) ||
            ...
            tester.insert_varying_sized_value<CassDuration>(CASS_VALUE_TYPE_DURATION, value, size,
                                                            nested_collection_type);
          }

          tester.insert_varying_sized_value<cass_double_t>(
              CASS_VALUE_TYPE_DOUBLE, 3.141592653589793, size, nested_collection_type);
          tester.insert_varying_sized_value<cass_float_t>(CASS_VALUE_TYPE_FLOAT, 3.1415926f, size,
                                                          nested_collection_type);
          tester.insert_varying_sized_value<cass_int32_t>(CASS_VALUE_TYPE_INT, 123, size,
                                                          nested_collection_type);
          if ((version.major_version >= 2 && version.minor_version >= 2) ||
              version.major_version >= 3) {
            tester.insert_varying_sized_value<cass_int16_t>(CASS_VALUE_TYPE_SMALL_INT, 123, size,
                                                            nested_collection_type);
            tester.insert_varying_sized_value<cass_int8_t>(CASS_VALUE_TYPE_TINY_INT, 123, size,
                                                           nested_collection_type);
            tester.insert_varying_sized_value<CassDate>(
                CASS_VALUE_TYPE_DATE, test_utils::Value<CassDate>::min_value() + 1u, size,
                nested_collection_type);
            tester.insert_varying_sized_value<CassTime>(CASS_VALUE_TYPE_TIME, 123, size,
                                                        nested_collection_type);
          }

          {
            CassUuid value = test_utils::generate_random_uuid(tester.uuid_gen);
            tester.insert_varying_sized_value<CassUuid>(CASS_VALUE_TYPE_UUID, value, size,
                                                        nested_collection_type);
          }

          {
            CassInet value = test_utils::inet_v4_from_int(16777343); // 127.0.0.1
            tester.insert_varying_sized_value<CassInet>(CASS_VALUE_TYPE_INET, value, size,
                                                        nested_collection_type);
          }

          {
            CassUuid value = test_utils::generate_time_uuid(tester.uuid_gen);
            tester.insert_varying_sized_value<CassUuid>(CASS_VALUE_TYPE_TIMEUUID, value, size,
                                                        nested_collection_type);
          }
        }
      }
    ...
    std::cout << "Unsupported Test for Cassandra v" << version.to_string()
              << ": Skipping tuples/varying_size" << std::endl;
    BOOST_REQUIRE(true);
  }
}
  ...
  if ((version.major_version >= 2 && version.minor_version >= 1) || version.major_version >= 3) {
    ...
    for (unsigned int i = 0; i < (sizeof(tester.sizes_) / sizeof(int)); ++i) {
      ...
      for (unsigned int j = 0;
           j < (sizeof(tester.nested_collection_types_) / sizeof(CassValueType)); ++j) {
        CassValueType nested_collection_type = tester.nested_collection_types_[j];
        tester.insert_varying_sized_null_value<CassString>(CASS_VALUE_TYPE_ASCII, size,
                                                           nested_collection_type);
        tester.insert_varying_sized_null_value<CassString>(
            CASS_VALUE_TYPE_VARCHAR, size,
            nested_collection_type); // NOTE: text is alias for varchar
        tester.insert_varying_sized_null_value<cass_int64_t>(CASS_VALUE_TYPE_BIGINT, size,
                                                             nested_collection_type);
        tester.insert_varying_sized_null_value<cass_int64_t>(CASS_VALUE_TYPE_TIMESTAMP, size,
                                                             nested_collection_type);
        tester.insert_varying_sized_null_value<CassBytes>(CASS_VALUE_TYPE_BLOB, size,
                                                          nested_collection_type);
        tester.insert_varying_sized_null_value<CassBytes>(CASS_VALUE_TYPE_VARINT, size,
                                                          nested_collection_type);
        tester.insert_varying_sized_null_value<cass_bool_t>(CASS_VALUE_TYPE_BOOLEAN, size,
                                                            nested_collection_type);
        tester.insert_varying_sized_null_value<CassDecimal>(CASS_VALUE_TYPE_DECIMAL, size,
                                                            nested_collection_type);
        tester.insert_varying_sized_null_value<cass_double_t>(CASS_VALUE_TYPE_DOUBLE, size,
                                                              nested_collection_type);
        tester.insert_varying_sized_null_value<cass_float_t>(CASS_VALUE_TYPE_FLOAT, size,
                                                             nested_collection_type);
        tester.insert_varying_sized_null_value<cass_int32_t>(CASS_VALUE_TYPE_INT, size,
                                                             nested_collection_type);
        tester.insert_varying_sized_null_value<CassUuid>(CASS_VALUE_TYPE_UUID, size,
                                                         nested_collection_type);
        tester.insert_varying_sized_null_value<CassInet>(CASS_VALUE_TYPE_INET, size,
                                                         nested_collection_type);
        tester.insert_varying_sized_null_value<CassUuid>(CASS_VALUE_TYPE_TIMEUUID, size,
                                                         nested_collection_type);
        if ((version.major_version >= 2 && version.minor_version >= 2) ||
            version.major_version >= 3) {
          tester.insert_varying_sized_null_value<cass_int8_t>(CASS_VALUE_TYPE_TINY_INT, size,
                                                              nested_collection_type);
          tester.insert_varying_sized_null_value<cass_int16_t>(CASS_VALUE_TYPE_SMALL_INT, size,
                                                               nested_collection_type);
          tester.insert_varying_sized_null_value<CassDate>(CASS_VALUE_TYPE_DATE, size,
                                                           nested_collection_type);
          tester.insert_varying_sized_null_value<CassTime>(CASS_VALUE_TYPE_TIME, size,
                                                           nested_collection_type);
        }
        if ((version.major_version >= 3 && version.minor_version >= 10) ||
            version.major_version >= 4) {
          tester.insert_varying_sized_null_value<CassDuration>(CASS_VALUE_TYPE_DURATION, size,
                                                               nested_collection_type);
        }
      }
    }
...
BOOST_AUTO_TEST_CASE(invalid) {
  ...
  if ((version.major_version >= 2 && version.minor_version >= 1) || version.major_version >= 3) {
    ...
    std::cout << "Unsupported Test for Cassandra v" << version.to_string()
              << ": Skipping tuples/invalid" << std::endl;
    BOOST_REQUIRE(true);
  }
}
