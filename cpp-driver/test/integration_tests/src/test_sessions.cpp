...
#define SESSION_STRESS_NUMBER_OF_SHARED_SESSION_THREADS \
  8 // NOTE: Total threads will be (SESSION_STRESS_NUMBER_OF_SESSIONS / 4) *
    // SESSION_STRESS_NUMBER_OF_SHARED_SESSION_THREADS
#define SESSION_STRESS_CHAOS_NUMBER_OF_ITERATIONS 256
#define SESSION_STRESS_NUMBER_OF_ITERATIONS \
  4 // NOTE: This effects sleep timer as well for async log messages
...
BOOST_AUTO_TEST_CASE(add_nodes_connect) {
  test_utils::CassLog::reset(SESSION_STRESS_OPENED_LOG_MESSAGE);
  {
    ...
    std::cout << "Create single node cluster with all three nodes initialized as contact points"
              << std::endl;
    test_utils::CassClusterPtr cluster(cass_cluster_new());
    if (ccm->create_cluster()) {
      ccm->start_cluster();
  ...
  test_utils::CassLog::reset(SESSION_STRESS_OPENED_LOG_MESSAGE);
  {
    ...
    std::cout
        << "Create single node cluster with two of the three nodes initialized as contact points"
        << std::endl;
    test_utils::CassClusterPtr cluster(cass_cluster_new());
    if (ccm->create_cluster()) {
      ccm->start_cluster();
  ...
  test_utils::CassLog::reset(SESSION_STRESS_OPENED_LOG_MESSAGE);
  {
    ...
    std::cout
        << "Create single node cluster with one of the three nodes initialized as contact points"
        << std::endl;
    test_utils::CassClusterPtr cluster(cass_cluster_new());
    if (ccm->create_cluster()) {
      ccm->start_cluster();
  ...
  test_utils::CassLog::reset(SESSION_STRESS_OPENED_LOG_MESSAGE);
  {
    ...
    std::cout << "Create two node cluster with all three of the nodes initialized as contact points"
              << std::endl;
    test_utils::CassClusterPtr cluster(cass_cluster_new());
  ...
  test_utils::CassLog::reset(SESSION_STRESS_OPENED_LOG_MESSAGE);
  {
    ...
    std::cout << "Create two node cluster with two of the three nodes initialized as contact points"
              << std::endl;
    test_utils::CassClusterPtr cluster(cass_cluster_new());
    if (ccm->create_cluster(2)) {
      ccm->start_cluster();
 ...
 */
void open_sessions(SessionContainer* sessions, unsigned int num_of_sessions,
                   bool is_concurrently = true) {
 ...
 * @param is_concurrently True if concurrent; false otherwise
 */
void close_sessions(SessionContainer* sessions, unsigned int num_of_sessions,
                    bool is_concurrently = true) {
  // Close session threads (LIFO)
  std::vector<uv_thread_t> threads(num_of_sessions);
  for (unsigned int n = 0; n < num_of_sessions; ++n) {
    if (is_concurrently) {
      uv_thread_create(&threads[n], close_session,
                       sessions->sessions[sessions->count() - n - 1].get());
    } else {
...
static void query_session(void* arg) {
  ...
  query->error_code = test_utils::execute_query_with_error(
      query->session, test_utils::SELECT_VERSION, NULL, CASS_CONSISTENCY_ONE,
...
void query_sessions(const SessionContainer& sessions) {
  ...
  for (unsigned int iterations = 0; iterations < SESSION_STRESS_NUMBER_OF_SHARED_SESSION_THREADS;
       ++iterations) {
  ...
  if (no_host_count > SESSION_STRESS_NUMBER_OF_ALLOWED_NO_HOST_AVAILABLE_OCCURRENCES) {
    BOOST_FAIL("Unacceptable Limit of CASS_ERROR_LIB_NO_HOSTS_AVAILABLE Occurred: "
               << no_host_count << " > "
 ...
 *     <li>Verify that SESSION_STRESS_NUMBER_OF_SESSIONS sessions are reported as open by the {@code
 * Session}</li> <li>Close SESSION_STRESS_NUMBER_OF_SESSIONS {@code Session} concurrently</li>
 *     <li>Verify that SESSION_STRESS_NUMBER_OF_SESSIONS sessions are reported as close by the
 * {@code Session}</li>
 *   </ol>
 *   <li>Open multiple sessions [SESSION_STRESS_NUMBER_OF_SESSIONS / 4] and perform query operations
 * sharing sessions on multiple threads</li> <li>Open multiple sessions
 * [SESSION_STRESS_NUMBER_OF_SESSIONS / 4] and perform query operations sharing sessions on multiple
 * threads with minor chaos</li> <ol> <li>Kill one node</li> <li>Disable gossip on another node</li>
...
BOOST_AUTO_TEST_CASE(stress) {
...

  {
    ...
    for (unsigned int iterations = 0; iterations < SESSION_STRESS_NUMBER_OF_ITERATIONS;
         ++iterations) {
    ...
    BOOST_CHECK_EQUAL(test_utils::CassLog::message_count(),
                      (SESSION_STRESS_NUMBER_OF_SESSIONS / 4));
