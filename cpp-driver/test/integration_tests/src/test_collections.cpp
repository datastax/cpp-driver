...
struct CollectionsTests : public test_utils::MultipleNodesTest {
  ...
  template <class T>
  void insert_collection_value(CassSession* session, CassValueType type, CassValueType primary_type,
                               const std::vector<T>& values) {
    std::string table_name = str(boost::format("%s_%s") % test_utils::SIMPLE_TABLE %
                                 test_utils::get_value_type(primary_type));
    std::string type_name = str(boost::format("%s<%s>") % test_utils::get_value_type(type) %
                                test_utils::get_value_type(primary_type));

    test_utils::execute_query(
        session, str(boost::format("CREATE TABLE %s (tweet_id int PRIMARY KEY, test_val %s);") %
                     table_name % type_name));

    test_utils::CassCollectionPtr input(
        cass_collection_new(static_cast<CassCollectionType>(type), values.size()));

    for (typename std::vector<T>::const_iterator it = values.begin(), end = values.end(); it != end;
         ++it) {
      test_utils::Value<T>::append(input.get(), *it);
    }

    std::string query =
        str(boost::format("INSERT INTO %s (tweet_id, test_val) VALUES(0, ?);") % table_name);
    ...
    test_utils::execute_query(
        session, str(boost::format("SELECT * FROM %s WHERE tweet_id = 0;") % table_name), &result);
    ...
    if (type == CASS_VALUE_TYPE_LIST) {
      ...
      while (cass_iterator_next(iterator.get())) {
        T result_value;
        BOOST_REQUIRE(test_utils::Value<T>::get(cass_iterator_get_value(iterator.get()),
                                                &result_value) == CASS_OK);
  ...
  void insert_collection_all_types(CassValueType type) {
    ...
    }
    {
      ...
      insert_collection_value<CassString>(session.get(), type, CASS_VALUE_TYPE_VARCHAR,
                                          values); // NOTE: text is alias for varchar
    }
    {
    ...
    if (type != CASS_VALUE_TYPE_SET &&
        ((version.major_version >= 3 && version.minor_version >= 10) ||
  ...
  void insert_map_value(CassSession* session, CassValueType primary_type,
                        CassValueType secondary_type, const std::map<K, V>& values) {
    std::string table_name =
        str(boost::format("%s_%s_%s") % test_utils::SIMPLE_TABLE %
            test_utils::get_value_type(primary_type) % test_utils::get_value_type(secondary_type));
    std::string type_name =
        str(boost::format("%s<%s, %s>") % test_utils::get_value_type(CASS_VALUE_TYPE_MAP) %
            test_utils::get_value_type(primary_type) % test_utils::get_value_type(secondary_type));
    ...
    test_utils::CassCollectionPtr input(
        cass_collection_new(CASS_COLLECTION_TYPE_MAP, values.size()));

    for (typename std::map<K, V>::const_iterator it = values.begin(), end = values.end(); it != end;
         ++it) {
    ...
    while (cass_iterator_next(iterator.get())) {
      ...
      BOOST_REQUIRE(test_utils::Value<K>::get(cass_iterator_get_value(iterator.get()),
                                              &result_key) == CASS_OK);
      BOOST_REQUIRE(cass_value_type(cass_iterator_get_value(iterator.get())) == primary_type);

      BOOST_REQUIRE(cass_iterator_next(iterator.get()) == cass_true);
      BOOST_REQUIRE(test_utils::Value<V>::get(cass_iterator_get_value(iterator.get()),
                                              &result_value) == CASS_OK);
  ...
  void insert_map_all_types() {
    ...
    if ((version.major_version >= 2 && version.minor_version >= 2) || version.major_version >= 3) {
      {
        ...
        values[3] = 4;
        values[5] = 6;
        insert_map_value<cass_int8_t, cass_int8_t>(session.get(), CASS_VALUE_TYPE_TINY_INT,
                                                   CASS_VALUE_TYPE_TINY_INT, values);
      }
      {
        ...
        values[3] = 4;
        values[5] = 6;
        insert_map_value<cass_int16_t, cass_int16_t>(session.get(), CASS_VALUE_TYPE_SMALL_INT,
                                                     CASS_VALUE_TYPE_SMALL_INT, values);
      }
      {
        ...
        values[min + 3] = min + 4;
        values[min + 5] = min + 6;
        insert_map_value<CassDate, CassDate>(session.get(), CASS_VALUE_TYPE_DATE,
                                             CASS_VALUE_TYPE_DATE, values);
      }
      {
        ...
        values[3] = 4;
        values[5] = 6;
        insert_map_value<CassTime, CassTime>(session.get(), CASS_VALUE_TYPE_TIME,
                                             CASS_VALUE_TYPE_TIME, values);
      }
    }

    {
      ...
      values[3] = 4;
      values[5] = 6;
      insert_map_value<cass_int32_t, cass_int32_t>(session.get(), CASS_VALUE_TYPE_INT,
                                                   CASS_VALUE_TYPE_INT, values);
    }

    {
      ...
      values[3LL] = 4LL;
      values[5LL] = 6LL;
      insert_map_value<cass_int64_t, cass_int64_t>(session.get(), CASS_VALUE_TYPE_BIGINT,
                                                   CASS_VALUE_TYPE_BIGINT, values);
    }

    {
      ...
      values[0.3f] = 0.4f;
      values[0.5f] = 0.6f;
      insert_map_value<cass_float_t, cass_float_t>(session.get(), CASS_VALUE_TYPE_FLOAT,
                                                   CASS_VALUE_TYPE_FLOAT, values);
    }

    {
      ...
      insert_map_value<cass_double_t, cass_double_t>(session.get(), CASS_VALUE_TYPE_DOUBLE,
                                                     CASS_VALUE_TYPE_DOUBLE, values);
    }

    {
      ...
      insert_map_value<CassString, CassString>(session.get(), CASS_VALUE_TYPE_VARCHAR,
                                               CASS_VALUE_TYPE_VARCHAR, values);
    }

    {
      ...
      insert_map_value<CassBytes, CassBytes>(session.get(), CASS_VALUE_TYPE_BLOB,
                                             CASS_VALUE_TYPE_BLOB, values);
    }

    {
      ...
      insert_map_value<CassInet, CassInet>(session.get(), CASS_VALUE_TYPE_INET,
                                           CASS_VALUE_TYPE_INET, values);
    }

    {
      ...
      values[test_utils::generate_time_uuid(uuid_gen)] = test_utils::generate_random_uuid(uuid_gen);
      values[test_utils::generate_time_uuid(uuid_gen)] = test_utils::generate_random_uuid(uuid_gen);
      insert_map_value<CassUuid, CassUuid>(session.get(), CASS_VALUE_TYPE_UUID,
                                           CASS_VALUE_TYPE_UUID, values);
    }

    {
      const cass_uint8_t varint1[] = { 57, 115, 235, 135, 229, 215, 8,   125,
                                       13, 43,  1,   25,  32,  135, 129, 180 };
      const cass_uint8_t varint2[] = { 112, 176, 158, 120, 246, 235, 29, 145,
                                       238, 50,  108, 239, 219, 100, 250 };
      ...
      insert_map_value<CassDecimal, CassDecimal>(session.get(), CASS_VALUE_TYPE_DECIMAL,
                                                 CASS_VALUE_TYPE_DECIMAL, values);
    }
    ...
    if ((version.major_version >= 3 && version.minor_version >= 10) || version.major_version >= 4) {
      ...
      insert_map_value<cass_int32_t, CassDuration>(session.get(), CASS_VALUE_TYPE_INT,
                                                   CASS_VALUE_TYPE_DURATION, values);
    }

    {
      ...
      insert_map_value<CassString, cass_int32_t>(session.get(), CASS_VALUE_TYPE_VARCHAR,
                                                 CASS_VALUE_TYPE_INT, values);
    }

    {
      ...
      insert_map_value<CassUuid, CassString>(session.get(), CASS_VALUE_TYPE_UUID,
                                             CASS_VALUE_TYPE_VARCHAR, values);
    }
...
BOOST_AUTO_TEST_CASE(set) { insert_collection_all_types(CASS_VALUE_TYPE_SET); }

BOOST_AUTO_TEST_CASE(list) { insert_collection_all_types(CASS_VALUE_TYPE_LIST); }

BOOST_AUTO_TEST_CASE(map) { insert_map_all_types(); }
