...
#define RETRIEVE_METADATA(meta, func, type)                                                    \
  do {                                                                                         \
    int attempts = 0;                                                                          \
...
struct TestSchemaMetadata : public test_utils::SingleSessionTest {
  ...
  void refresh_schema_meta() {
    if (schema_meta_) {
      ...
      for (size_t i = 0; i < 10 && cass_schema_meta_snapshot_version(schema_meta_) ==
                                       cass_schema_meta_snapshot_version(old);
           ++i) {
        boost::this_thread::sleep_for(boost::chrono::milliseconds(1000));
        cass_schema_meta_free(schema_meta_);
        schema_meta_ = cass_session_get_schema_meta(session);
      }
      if (cass_schema_meta_snapshot_version(schema_meta_) ==
          cass_schema_meta_snapshot_version(old)) {
  ...
  void create_simple_strategy_keyspace(unsigned int replication_factor = 1,
                                       bool durable_writes = true) {
    test_utils::execute_query_with_error(
        session,
        str(boost::format(test_utils::DROP_KEYSPACE_FORMAT) % SIMPLE_STRATEGY_KEYSPACE_NAME));
    test_utils::execute_query(
        session,
        str(boost::format("CREATE KEYSPACE %s WITH replication = { 'class' : 'SimpleStrategy', "
                          "'replication_factor' : %s } AND durable_writes = %s") %
            SIMPLE_STRATEGY_KEYSPACE_NAME % replication_factor %
            (durable_writes ? "true" : "false")));
    refresh_schema_meta();
  }

  void create_network_topology_strategy_keyspace(unsigned int replicationFactorDataCenterOne = 3,
                                                 unsigned int replicationFactorDataCenterTwo = 2,
                                                 bool isDurableWrites = true) {
    test_utils::execute_query_with_error(
        session,
        str(boost::format(test_utils::DROP_KEYSPACE_FORMAT) % NETWORK_TOPOLOGY_KEYSPACE_NAME));
    test_utils::execute_query(
        session,
        str(boost::format(
                "CREATE KEYSPACE %s WITH replication = { 'class' : 'NetworkTopologyStrategy',  "
                "'dc1' : %d, 'dc2' : %d } AND durable_writes = %s") %
            NETWORK_TOPOLOGY_KEYSPACE_NAME % replicationFactorDataCenterOne %
            replicationFactorDataCenterTwo % (isDurableWrites ? "true" : "false")));
  ...
  const CassKeyspaceMeta* schema_get_keyspace(const std::string& ks_name) {
    ...
    RETRIEVE_METADATA(ks_meta, cass_schema_meta_keyspace_by_name(schema_meta_, ks_name.c_str()),
                      "Keyspace");
    BOOST_REQUIRE(ks_meta);
    return ks_meta;
  }

  const CassTableMeta* schema_get_table(const std::string& ks_name, const std::string& table_name) {
    const CassTableMeta* table_meta = NULL;
    RETRIEVE_METADATA(
        table_meta,
        cass_keyspace_meta_table_by_name(schema_get_keyspace(ks_name), table_name.c_str()),
        "Table");
  ...
  const CassColumnMeta* schema_get_column(const std::string& ks_name, const std::string& table_name,
                                          const std::string& col_name) {
    const CassColumnMeta* col_meta = NULL;
    RETRIEVE_METADATA(
        col_meta,
        cass_table_meta_column_by_name(schema_get_table(ks_name, table_name), col_name.c_str()),
        "Column");
  ...
  const CassFunctionMeta* schema_get_function(const std::string& ks_name,
    ...
    RETRIEVE_METADATA(
        func_meta,
        cass_keyspace_meta_function_by_name(schema_get_keyspace(ks_name), func_name.c_str(),
  ...
  const CassAggregateMeta* schema_get_aggregate(const std::string& ks_name,
    ...
    RETRIEVE_METADATA(
        agg_meta,
        cass_keyspace_meta_aggregate_by_name(schema_get_keyspace(ks_name), agg_name.c_str(),
  ...
  void verify_function_dropped(const std::string& ks_name, const std::string& func_name,
                               const std::vector<std::string>& func_types) {
    const CassFunctionMeta* func_meta =
        cass_keyspace_meta_function_by_name(schema_get_keyspace(ks_name), func_name.c_str(),
                                            test_utils::implode(func_types, ',').c_str());
    BOOST_CHECK(func_meta == NULL);
  }

  void verify_aggregate_dropped(const std::string& ks_name, const std::string& agg_name,
                                const std::vector<std::string>& agg_types) {
    const CassAggregateMeta* agg_meta =
        cass_keyspace_meta_aggregate_by_name(schema_get_keyspace(ks_name), agg_name.c_str(),
                                             test_utils::implode(agg_types, ',').c_str());
    BOOST_CHECK(agg_meta == NULL);
  }

  void verify_fields(const test_utils::CassIteratorPtr& itr,
    ...
    BOOST_REQUIRE_EQUAL_COLLECTIONS(observed.begin(), observed.end(), expected_fields.begin(),
                                    expected_fields.end());
  }
  ...
  void verify_value(const CassValue* value, const std::string& expected) {
    ...
    BOOST_REQUIRE(type == CASS_VALUE_TYPE_ASCII || type == CASS_VALUE_TYPE_TEXT ||
  ...
  void verify_value(const CassValue* value, std::map<std::string, value_type> expected) {
    ...
    while (cass_iterator_next(itr.get())) {
      ...
      typename std::map<std::string, value_type>::const_iterator i =
          expected.find(std::string(key.data, key.length));
      BOOST_REQUIRE(i != expected.end());
      verify_value(cass_iterator_get_map_value(itr.get()), i->second);
    }
  }

  void verify_index(const CassIndexMeta* index_meta, const std::string& index_name,
                    CassIndexType index_type, const std::string& index_target,
  ...
  void verify_fields_by_name(const CassKeyspaceMeta* keyspace_meta,
  ...
  void verify_fields_by_name(const CassColumnMeta* column_meta,
                             const std::set<std::string>& fields) {
    for (std::set<std::string>::iterator i = fields.begin(); i != fields.end(); ++i) {
  ...
  void verify_columns(const CassTableMeta* table_meta) {
    ...
    while (cass_iterator_next(itr.get())) {
      ...
      if (version >= "2.0.0" || (col_type != CASS_COLUMN_TYPE_CLUSTERING_KEY &&
                                 col_type != CASS_COLUMN_TYPE_PARTITION_KEY)) {
        verify_fields(test_utils::CassIteratorPtr(cass_iterator_fields_from_column_meta(col_meta)),
                      column_fields());
  ...
  void verify_table(const std::string& ks_name, const std::string& table_name,
    ...
    const CassTableMeta* table_meta =
        schema_get_table(SIMPLE_STRATEGY_KEYSPACE_NAME, ALL_DATA_TYPES_TABLE_NAME);

    verify_fields(test_utils::CassIteratorPtr(cass_iterator_fields_from_table_meta(table_meta)),
                  table_fields());
    verify_fields_by_name(table_meta, table_fields());
    verify_value(cass_table_meta_field_by_name(table_meta, "keyspace_name"),
                 SIMPLE_STRATEGY_KEYSPACE_NAME);
    verify_value(cass_table_meta_field_by_name(
                     table_meta, version >= "3.0.0" ? "table_name" : "columnfamily_name"),
                 ALL_DATA_TYPES_TABLE_NAME);

    // not going for every field, just making sure one of each type (fixed, list, map) is correctly
    // added
    verify_value(cass_table_meta_field_by_name(table_meta, "comment"), COMMENT);

    const CassValue* value = cass_table_meta_field_by_name(
        table_meta, version >= "3.0.0" ? "compression" : "compression_parameters");
    BOOST_REQUIRE(value);
    BOOST_REQUIRE_EQUAL(cass_value_type(value), CASS_VALUE_TYPE_MAP);
    ...
    // new column
    test_utils::execute_query(session,
                              "ALTER TABLE " + table_name + " ADD jkldsfafdjsklafajklsljkfds text");
    refresh_schema_meta();
    table_meta = schema_get_table(SIMPLE_STRATEGY_KEYSPACE_NAME, ALL_DATA_TYPES_TABLE_NAME);
    ...
    refresh_schema_meta();
    const CassKeyspaceMeta* ks_meta =
        cass_schema_meta_keyspace_by_name(schema_meta_, SIMPLE_STRATEGY_KEYSPACE_NAME);
    table_meta = cass_keyspace_meta_table_by_name(ks_meta, ALL_DATA_TYPES_TABLE_NAME);
    BOOST_CHECK(!table_meta);
  }

  void verify_materialized_view_count(const std::string& keyspace_name, size_t count) {
    ...
    while (actual_count != count && attempts < 10) {
      ...
      test_utils::CassIteratorPtr iterator(
          cass_iterator_materialized_views_from_keyspace_meta(keyspace_meta));
      while (cass_iterator_next(iterator.get())) {
        actual_count++;
      }
  ...
  void verify_materialized_view(const CassMaterializedViewMeta* view, const std::string& view_name,
    ...
    for (std::vector<std::string>::const_iterator i = columns.begin(), end = columns.end();
         i != end; ++i) {
    ...
    BOOST_REQUIRE_EQUAL(cass_materialized_view_meta_partition_key_count(view),
                        partition_key.size());
    ...
    BOOST_REQUIRE_EQUAL(cass_materialized_view_meta_clustering_key_count(view),
                        clustering_key.size());
  ...
  void verify_keyspace(const std::string& name, bool durable_writes,
    ...
    verify_fields(test_utils::CassIteratorPtr(cass_iterator_fields_from_keyspace_meta(ks_meta)),
                  keyspace_fields());
    verify_fields_by_name(ks_meta, keyspace_fields());
    verify_value(cass_keyspace_meta_field_by_name(ks_meta, "keyspace_name"), name);
    // verify_value(cass_keyspace_meta_field_by_name(ks_meta, "durable_writes"),
  ...
  void verify_user_keyspace() {
    ...
    verify_keyspace(SIMPLE_STRATEGY_KEYSPACE_NAME, true, SIMPLE_STRATEGY_CLASS_NAME,
                    strategy_options);

    // alter keyspace
    test_utils::execute_query(session, "ALTER KEYSPACE " SIMPLE_STRATEGY_KEYSPACE_NAME
                                       " WITH replication = { 'class' : 'SimpleStrategy', "
                                       "'replication_factor': '2' } AND durable_writes = false");
    refresh_schema_meta();

    strategy_options["replication_factor"] = "2";
    verify_keyspace(SIMPLE_STRATEGY_KEYSPACE_NAME, false, SIMPLE_STRATEGY_CLASS_NAME,
                    strategy_options);
    ...
    verify_keyspace(NETWORK_TOPOLOGY_KEYSPACE_NAME, true, NETWORK_TOPOLOGY_STRATEGY_CLASS_NAME,
                    strategy_options);
  ...
  void verify_user_table() {
    ...
    test_utils::execute_query(session, str(boost::format(test_utils::CREATE_TABLE_ALL_TYPES) %
                                           ALL_DATA_TYPES_TABLE_NAME));
    refresh_schema_meta();
    test_utils::execute_query(session, "ALTER TABLE " ALL_DATA_TYPES_TABLE_NAME
                                       " WITH comment='" COMMENT "'");
    refresh_schema_meta();

    verify_table(SIMPLE_STRATEGY_KEYSPACE_NAME, ALL_DATA_TYPES_TABLE_NAME, COMMENT,
                 "boolean_sample");
  }

  std::vector<std::string> get_user_data_type_field_names(const std::string& ks_name,
    ...
    if (ks_meta) {
      const CassDataType* data_type =
          cass_keyspace_meta_user_type_by_name(ks_meta, udt_name.c_str());
      if (data_type) {
  ...
  void verify_user_type(const std::string& ks_name, const std::string& udt_name,
                        const std::vector<std::string>& udt_datatypes) {
    std::vector<std::string> udt_field_names =
        get_user_data_type_field_names(ks_name.c_str(), udt_name.c_str());
    BOOST_REQUIRE_EQUAL_COLLECTIONS(udt_datatypes.begin(), udt_datatypes.end(),
                                    udt_field_names.begin(), udt_field_names.end());
  }

  void verify_partition_key(const CassTableMeta* table_meta, size_t index,
  ...
  void verify_clustering_key(const CassTableMeta* table_meta, size_t index,
                             const std::string& column_name) {
    CassString actual_name;
  ...
  void verify_column_order(const CassTableMeta* table_meta, size_t partition_key_size,
                           size_t clustering_key_size, size_t column_count) {
    ...
    for (; index < clustering_key_size; ++index) {
      ...
      RETRIEVE_METADATA(column_meta, cass_table_meta_column(table_meta, index), "Column");
      BOOST_REQUIRE(column_meta);
      BOOST_CHECK_EQUAL(cass_column_meta_type(column_meta), CASS_COLUMN_TYPE_CLUSTERING_KEY);
    }
  ...
  void verify_user_function(const std::string& ks_name, const std::string& udf_name,
                            const std::vector<std::string>& udf_argument,
                            const std::vector<std::string>& udf_value_types,
                            const std::string& udf_body, const std::string& udf_language,
    ...
    // Function name
    cass_function_meta_name(func_meta, &func_meta_string_value.data,
                            &func_meta_string_value.length);
    BOOST_CHECK(
        test_utils::Value<CassString>::equal(CassString(udf_name.c_str()), func_meta_string_value));
    ...
    cass_function_meta_full_name(func_meta, &func_meta_string_value.data,
                                 &func_meta_string_value.length);
    BOOST_CHECK(test_utils::Value<CassString>::equal(CassString(udf_full_name.str().c_str()),
                                                     func_meta_string_value));

    // Function body
    cass_function_meta_body(func_meta, &func_meta_string_value.data,
                            &func_meta_string_value.length);
    BOOST_CHECK(
        test_utils::Value<CassString>::equal(CassString(udf_body.c_str()), func_meta_string_value));

    // Function language
    cass_function_meta_language(func_meta, &func_meta_string_value.data,
                                &func_meta_string_value.length);
    BOOST_CHECK(test_utils::Value<CassString>::equal(CassString(udf_language.c_str()),
                                                     func_meta_string_value));
    ...
    for (size_t i = 0; i < udf_argument.size(); ++i) {
      ...
      cass_function_meta_argument(func_meta, i, &func_meta_string_value.data,
                                  &func_meta_string_value.length, &datatype);
      BOOST_CHECK_EQUAL(udf_value_types[i].compare(
                            std::string(test_utils::get_value_type(cass_data_type_type(datatype)))),
                        0);
    }

    // Function arguments (by name)
    std::vector<std::string>::const_iterator value_iterator = udf_value_types.begin();
    for (std::vector<std::string>::const_iterator iterator = udf_argument.begin();
         iterator != udf_argument.end(); ++iterator) {
      const CassDataType* datatype =
          cass_function_meta_argument_type_by_name(func_meta, (*iterator).c_str());
      BOOST_CHECK_EQUAL(
          (*value_iterator)
              .compare(std::string(test_utils::get_value_type(cass_data_type_type(datatype)))),
          0);
      ++value_iterator;
    }
  ...
  template <class T>
  void verify_user_aggregate(const std::string& ks_name, const std::string& udf_name,
                             std::string udf_final_name, const std::string& uda_name,
                             const std::vector<std::string>& uda_value_types,
                             const CassValueType return_value_type,
                             const CassValueType state_value_type, T init_cond_value) {
    ...
    BOOST_CHECK(
        test_utils::Value<CassString>::equal(CassString(uda_name.c_str()), agg_meta_string_value));
    ...
    cass_aggregate_meta_full_name(agg_meta, &agg_meta_string_value.data,
                                  &agg_meta_string_value.length);
    BOOST_CHECK(test_utils::Value<CassString>::equal(CassString(uda_full_name.str().c_str()),
                                                     agg_meta_string_value));
    ...
    for (size_t i = 0; i < uda_value_types.size(); ++i) {
      ...
      BOOST_CHECK_EQUAL(uda_value_types[i].compare(
                            std::string(test_utils::get_value_type(cass_data_type_type(datatype)))),
                        0);
    }
    ...
    BOOST_CHECK(
        test_utils::Value<CassString>::equal(CassString(udf_name.c_str()), agg_meta_string_value));

    // Aggregate final function
    func_meta = cass_aggregate_meta_final_func(agg_meta);
    cass_function_meta_name(func_meta, &agg_meta_string_value.data, &agg_meta_string_value.length);
    BOOST_CHECK(test_utils::Value<CassString>::equal(CassString(udf_final_name.c_str()),
                                                     agg_meta_string_value));
    ...
    if (version >= "3.0.0") {
      ...
      BOOST_CHECK(test_utils::Value<CassString>::get(agg_init_cond, &agg_init_cond_value) ==
                  CASS_OK);
      BOOST_REQUIRE(test_utils::Value<CassString>::equal(CassString(s.data(), s.length()),
                                                         agg_init_cond_value));
    } else {
  ...
  void verify_user_data_type() {
    ...
    // New UDT
    test_utils::execute_query(
        session, str(boost::format("CREATE TYPE %s(integer_value int)") % USER_DATA_TYPE_NAME));
    ...
    // Altered UDT
    test_utils::execute_query(
        session, str(boost::format("ALTER TYPE %s ADD text_value text") % USER_DATA_TYPE_NAME));
  ...
  void create_simple_strategy_functions() {
    test_utils::execute_query(
        session,
        str(boost::format("CREATE OR REPLACE FUNCTION %s.%s(rhs int, lhs int) RETURNS NULL ON NULL "
                          "INPUT RETURNS int LANGUAGE javascript AS 'lhs + rhs';") %
            SIMPLE_STRATEGY_KEYSPACE_NAME % USER_DEFINED_FUNCTION_NAME));
  }
  ...
  void verify_user_defined_function() {
    ...
    udf_value_types.push_back(test_utils::get_value_type(CASS_VALUE_TYPE_INT));
    refresh_schema_meta();
    verify_user_function(SIMPLE_STRATEGY_KEYSPACE_NAME, USER_DEFINED_FUNCTION_NAME, udf_arguments,
                         udf_value_types, test_utils::implode(udf_arguments, '+', " ", " "),
                         "javascript", cass_false, CASS_VALUE_TYPE_INT);

    // Drop UDF
    test_utils::execute_query(session,
                              str(boost::format("DROP FUNCTION %s.%s") %
                                  SIMPLE_STRATEGY_KEYSPACE_NAME % USER_DEFINED_FUNCTION_NAME));
    refresh_schema_meta();
    verify_function_dropped(SIMPLE_STRATEGY_KEYSPACE_NAME, USER_DEFINED_FUNCTION_NAME,
                            udf_value_types);
  }
  ...
  void create_simple_strategy_aggregate() {
    create_simple_strategy_functions();
    test_utils::execute_query(
        session, str(boost::format("CREATE OR REPLACE FUNCTION %s.%s(val int) RETURNS NULL ON NULL "
                                   "INPUT RETURNS int LANGUAGE javascript AS 'val * val';") %
                     SIMPLE_STRATEGY_KEYSPACE_NAME % USER_DEFINED_AGGREGATE_FINAL_FUNCTION_NAME));

    test_utils::execute_query(
        session, str(boost::format("CREATE OR REPLACE AGGREGATE %s.%s(int) SFUNC %s STYPE int "
                                   "FINALFUNC %s INITCOND 0") %
                     SIMPLE_STRATEGY_KEYSPACE_NAME % USER_DEFINED_AGGREGATE_NAME %
  ...
  void verify_user_defined_aggregate() {
    ...
    refresh_schema_meta();
    verify_user_aggregate<cass_int32_t>(SIMPLE_STRATEGY_KEYSPACE_NAME, USER_DEFINED_FUNCTION_NAME,
                                        USER_DEFINED_AGGREGATE_FINAL_FUNCTION_NAME,
                                        USER_DEFINED_AGGREGATE_NAME, uda_value_types,
                                        CASS_VALUE_TYPE_INT, CASS_VALUE_TYPE_INT, 0);

    // Drop UDA
    test_utils::execute_query(session,
                              str(boost::format("DROP AGGREGATE %s.%s") %
                                  SIMPLE_STRATEGY_KEYSPACE_NAME % USER_DEFINED_AGGREGATE_NAME));
    refresh_schema_meta();
    verify_aggregate_dropped(SIMPLE_STRATEGY_KEYSPACE_NAME, USER_DEFINED_AGGREGATE_NAME,
                             uda_value_types);
  }
};
...
BOOST_AUTO_TEST_CASE(keys) {
  ...
  {
    test_utils::execute_query(
        session, "CREATE TABLE keys.single_partition_key (key text, value text, PRIMARY KEY(key))");
    refresh_schema_meta();
  ...
  {
    test_utils::execute_query(
        session, "CREATE TABLE keys.composite_partition_key (key1 text, key2 text, value text, "
  ...
  {
    test_utils::execute_query(session,
                              "CREATE TABLE keys.composite_key (key1 text, key2 text, value text, "
                              "PRIMARY KEY(key1, key2))");
    refresh_schema_meta();
...

  {
    test_utils::execute_query(
        session,
        "CREATE TABLE keys.composite_clustering_key (key1 text, key2 text, key3 text, value text, "
  ...
  {
    test_utils::execute_query(session, "CREATE TABLE keys.composite_partition_and_clustering_key "
                                       "(key1 text, key2 text, key3 text, key4 text, value text, "
                                       "PRIMARY KEY((key1, key2), key3, key4))");
    refresh_schema_meta();

    const CassTableMeta* table_meta =
        schema_get_table("keys", "composite_partition_and_clustering_key");
...
BOOST_AUTO_TEST_CASE(dense_table) {
  ...
  test_utils::execute_query(session, "CREATE TABLE dense.my_table (key text, value text, PRIMARY "
                                     "KEY(key, value)) WITH COMPACT STORAGE");
  refresh_schema_meta();
...
BOOST_AUTO_TEST_CASE(disable) {
  ...
  test_utils::execute_query_with_error(
      session, str(boost::format(test_utils::DROP_KEYSPACE_FORMAT) % "ks1"));
  test_utils::execute_query_with_error(
      session, str(boost::format(test_utils::DROP_KEYSPACE_FORMAT) % "ks2"));
}
...
BOOST_AUTO_TEST_CASE(clustering_order) {
  ...
  {
    test_utils::execute_query(session, "CREATE TABLE clustering_order.single_partition_key (key "
                                       "text, value text, PRIMARY KEY(key))");
    refresh_schema_meta();
    ...
    BOOST_CHECK_EQUAL(cass_table_meta_clustering_key_order(table_meta, 0),
                      CASS_CLUSTERING_ORDER_NONE);
  }

  {
    test_utils::execute_query(
        session, "CREATE TABLE clustering_order.composite_key (key1 int, key2 text, value text, "
                 "PRIMARY KEY(key1, key2))");
    refresh_schema_meta();
  ...
  {
    test_utils::execute_query(session, "CREATE TABLE clustering_order.composite_clustering_key "
                                       "(key1 text, key2 text, key3 text, value text, "
                                       "PRIMARY KEY(key1, key2, key3))");
    refresh_schema_meta();

    const CassTableMeta* table_meta =
        schema_get_table("clustering_order", "composite_clustering_key");

    BOOST_REQUIRE_EQUAL(cass_table_meta_clustering_key_count(table_meta), 2);
    BOOST_CHECK_EQUAL(cass_table_meta_clustering_key_order(table_meta, 0),
                      CASS_CLUSTERING_ORDER_ASC);
    BOOST_CHECK_EQUAL(cass_table_meta_clustering_key_order(table_meta, 1),
                      CASS_CLUSTERING_ORDER_ASC);
  }

  {
    test_utils::execute_query(
        session,
        "CREATE TABLE clustering_order.reversed_composite_key (key1 text, key2 text, value text, "
        "PRIMARY KEY(key1, key2)) "
        "WITH CLUSTERING ORDER BY (key2 DESC)");
    refresh_schema_meta();

    const CassTableMeta* table_meta =
        schema_get_table("clustering_order", "reversed_composite_key");
  ...
  {
    test_utils::execute_query(session,
                              "CREATE TABLE clustering_order.reversed_composite_clustering_key "
                              "(key1 text, key2 text, key3 text, value text, "
                              "PRIMARY KEY(key1, key2, key3))"
    ...
    const CassTableMeta* table_meta =
        schema_get_table("clustering_order", "reversed_composite_clustering_key");
  ...
  {
    test_utils::execute_query(session,
                              "CREATE TABLE clustering_order.mixed_composite_clustering_key ("
                              "key1 text, key2 text, key3 text, key4 text, value text, "
                              "PRIMARY KEY(key1, key2, key3, key4))"
    ...
    const CassTableMeta* table_meta =
        schema_get_table("clustering_order", "mixed_composite_clustering_key");
    ...
    BOOST_CHECK_EQUAL(cass_table_meta_clustering_key_order(table_meta, 1),
                      CASS_CLUSTERING_ORDER_ASC);
    BOOST_CHECK_EQUAL(cass_table_meta_clustering_key_order(table_meta, 2),
                      CASS_CLUSTERING_ORDER_DESC);
  }

  {
    test_utils::execute_query(session,
                              "CREATE TABLE clustering_order.mixed_order_composite_clustering_key ("
                              "key1 text, key2 text, key3 text, key4 text, value text, "
    ...
    const CassTableMeta* table_meta =
        schema_get_table("clustering_order", "mixed_order_composite_clustering_key");
...
BOOST_AUTO_TEST_CASE(frozen_types) {
  ...
  {
    test_utils::execute_query(session, "CREATE TABLE frozen_types.frozen_map (key text PRIMARY "
                                       "KEY, value frozen<map<text, text>>)");
    refresh_schema_meta();
  ...
  {
    test_utils::execute_query(
        session, "CREATE TABLE frozen_types.regular_set (key text PRIMARY KEY, value set<text>)");
    refresh_schema_meta();
  ...
  {
    test_utils::execute_query(
        session, "CREATE TABLE frozen_types.regular_list (key text PRIMARY KEY, value list<text>)");
    refresh_schema_meta();
  ...
  {
    test_utils::execute_query(session, "CREATE TABLE frozen_types.frozen_tuple (key text PRIMARY "
                                       "KEY, value frozen<tuple<text, int>>)");
    refresh_schema_meta();
  ...
  {
    test_utils::execute_query(
        session, "CREATE TYPE frozen_types.type1 (field1 text, field2 frozen<set<text>>)");
    test_utils::execute_query(
        session,
    ...
    BOOST_CHECK_EQUAL(cass_data_type_is_frozen(value_data_type),
                      version < "3.0.0" ? cass_false : cass_true);
  }

  {
    test_utils::execute_query(session,
                              "CREATE TABLE frozen_types.frozen_nested_map (key text PRIMARY KEY, "
                              "value map<frozen<set<text>>, frozen<list<text>>>)");
    refresh_schema_meta();

    const CassColumnMeta* column_meta =
        schema_get_column("frozen_types", "frozen_nested_map", "value");
    const CassDataType* data_type = cass_column_meta_data_type(column_meta);
  ...
  {
    test_utils::execute_query(
        session, "CREATE TABLE frozen_types.frozen_nested_tuple (key text PRIMARY KEY, "
                 "value tuple<int, text, frozen<set<text>>, frozen<list<text>>>)");
    refresh_schema_meta();

    const CassColumnMeta* column_meta =
        schema_get_column("frozen_types", "frozen_nested_tuple", "value");
    const CassDataType* data_type = cass_column_meta_data_type(column_meta);
    ...
    BOOST_CHECK_EQUAL(cass_data_type_is_frozen(key_data_type),
                      version < "3.0.0" ? cass_false : cass_true);
...
BOOST_AUTO_TEST_CASE(lookup) {
  ...
  // and https://docs.datastax.com/en/cql/3.3/cql/cql_using/useCreateUDA.html
  {
    ...
    test_utils::execute_query(
        session, "CREATE OR REPLACE FUNCTION lookup.avg_state(state tuple<int, bigint>, val int) "
                 "CALLED ON NULL INPUT RETURNS tuple<int, bigint> "
                 "LANGUAGE java AS 'if (val !=null) { state.setInt(0, state.getInt(0) + 1); "
    ...
    const CassFunctionMeta* func_meta = cass_keyspace_meta_function_by_name(
        schema_get_keyspace("lookup"), "avg_state", func_args.c_str());
    BOOST_REQUIRE(func_meta);
    ...
    BOOST_CHECK_EQUAL(CASS_VALUE_TYPE_INT,
                      cass_data_type_type(cass_data_type_sub_data_type(datatype, 0)));
    BOOST_CHECK_EQUAL(CASS_VALUE_TYPE_BIGINT,
    ...
    test_utils::execute_query(
        session, "CREATE OR REPLACE FUNCTION lookup.avg_final(state tuple<int, bigint>) "
                 "CALLED ON NULL INPUT RETURNS double "
                 "LANGUAGE java AS 'double r = 0; if (state.getInt(0) == 0) return null; r = "
    ...
    const CassAggregateMeta* agg_meta =
        cass_keyspace_meta_aggregate_by_name(schema_get_keyspace("lookup"), "average", "int");
    BOOST_REQUIRE(agg_meta);
...
BOOST_AUTO_TEST_CASE(indexes) {
  {
    ...
    test_utils::execute_query(session, "CREATE TABLE indexes.table1 (key1 text, value1 int, value2 "
                                       "map<text, text>,  PRIMARY KEY(key1))");
  ...
  // Index
  {
    ...
    verify_index(cass_table_meta_index_by_name(table_meta, "index1"), "index1",
                 CASS_INDEX_TYPE_COMPOSITES, "value1", index_options);
    verify_index(cass_table_meta_index(table_meta, 0), "index1", CASS_INDEX_TYPE_COMPOSITES,
                 "value1", index_options);
  }
  ...
  // Index on map keys (C* >= 2.1)
  {
    if (version >= "2.1.0") {
      ...
      verify_index(cass_table_meta_index_by_name(table_meta, "index2"), "index2",
                   CASS_INDEX_TYPE_COMPOSITES, "keys(value2)", index_options);
      verify_index(cass_table_meta_index(table_meta, 1), "index2", CASS_INDEX_TYPE_COMPOSITES,
                   "keys(value2)", index_options);
    }
  }
...
BOOST_AUTO_TEST_CASE(materialized_views) {
  {
    ...
    test_utils::execute_query(session, "CREATE TABLE materialized_views.table2 (key1 text, key2 "
                                       "int, value1 int, PRIMARY KEY(key1, key2))");
  ...
  {
    test_utils::execute_query(session,
                              "CREATE MATERIALIZED VIEW materialized_views.view1 AS "
    ...
    verify_materialized_view(cass_table_meta_materialized_view_by_name(table_meta, "view1"),
                             "view1", "table1", "value1,key1", "value1", "key1");
  }
  ...
  // Materialized view with composite partition key
  {
    test_utils::execute_query(
        session,
        "CREATE MATERIALIZED VIEW materialized_views.view2 AS "
    ...
    verify_materialized_view(cass_table_meta_materialized_view_by_name(table_meta, "view2"),
                             "view2", "table2", "value1,key2,key1", "value1,key2", "key1");
  }
  ...
  // Materialized view with composite clustering key
  {
    test_utils::execute_query(
        session,
        "CREATE MATERIALIZED VIEW materialized_views.view3 AS "
        "SELECT key1 FROM materialized_views.table2 WHERE key2 IS NOT NULL AND value1 IS NOT NULL "
        "PRIMARY KEY(value1, key2, key1) "
        "WITH CLUSTERING ORDER BY (key2 DESC)");
    ...
    verify_materialized_view(cass_table_meta_materialized_view_by_name(table_meta, "view3"),
                             "view3", "table2", "value1,key2,key1", "value1", "key2,key1");
  }
  ...
  // Iterator
  {
    ...
    test_utils::CassIteratorPtr iterator(
        cass_iterator_materialized_views_from_table_meta(table_meta));

    while (cass_iterator_next(iterator.get())) {
      const CassMaterializedViewMeta* view_meta =
          cass_iterator_get_materialized_view_meta(iterator.get());
      ...
      if (name == CassString("view2")) {
        verify_materialized_view(view_meta, "view2", "table2", "value1,key2,key1", "value1,key2",
                                 "key1");
      } else if (name == CassString("view3")) {
        verify_materialized_view(view_meta, "view3", "table2", "value1,key2,key1", "value1",
                                 "key2,key1");
      } else {
        BOOST_CHECK(false);
      }
  ...
  // (CPP-503: Schema metadata race condition when a view is dropped)
  {
    ...
    refresh_schema_meta();
    verify_materialized_view_count("materialized_views", 2);
    BOOST_REQUIRE(cass_keyspace_meta_materialized_view_by_name(
                      schema_get_keyspace("materialized_views"), "view2") == NULL);
    ...
    verify_materialized_view_count("materialized_views", 1);
    BOOST_REQUIRE(cass_keyspace_meta_materialized_view_by_name(
                      schema_get_keyspace("materialized_views"), "view1") == NULL);
...
BOOST_AUTO_TEST_CASE(materialized_view_clustering_order) {
  ...
  {
    test_utils::execute_query(
        session, "CREATE KEYSPACE materialized_view_clustering_order WITH replication = "
                 "{ 'class' : 'SimpleStrategy', 'replication_factor' : 3 }");

    test_utils::execute_query(session, "CREATE TABLE materialized_view_clustering_order.table1 "
                                       "(key1 text, value1 text, PRIMARY KEY(key1))");
    test_utils::execute_query(session,
                              "CREATE TABLE materialized_view_clustering_order.table2 (key1 text, "
                              "key2 text, value1 text, PRIMARY KEY(key1, key2))");
...

  {
    test_utils::execute_query(
        session,
        "CREATE MATERIALIZED VIEW materialized_view_clustering_order.composite_key AS "
        "SELECT key1 FROM materialized_view_clustering_order.table1 WHERE value1 IS NOT NULL "
        "PRIMARY KEY(value1, key1)");
    refresh_schema_meta();

    const CassMaterializedViewMeta* view_meta =
        schema_get_view("materialized_view_clustering_order", "composite_key");

    BOOST_REQUIRE_EQUAL(cass_materialized_view_meta_clustering_key_count(view_meta), 1);
    BOOST_CHECK_EQUAL(cass_materialized_view_meta_clustering_key_order(view_meta, 0),
                      CASS_CLUSTERING_ORDER_ASC);
  }

  {
    test_utils::execute_query(
        session,
        "CREATE MATERIALIZED VIEW materialized_view_clustering_order.reversed_composite_key AS "
        "SELECT key1 FROM materialized_view_clustering_order.table1 WHERE value1 IS NOT NULL "
        "PRIMARY KEY(value1, key1)"
    ...
    const CassMaterializedViewMeta* view_meta =
        schema_get_view("materialized_view_clustering_order", "reversed_composite_key");
...

  {
    test_utils::execute_query(
        session,
        "CREATE MATERIALIZED VIEW materialized_view_clustering_order.composite_clustering_key AS "
        "SELECT key1 FROM materialized_view_clustering_order.table2 WHERE key2 IS NOT NULL AND "
        "value1 IS NOT NULL "
        "PRIMARY KEY(value1, key2, key1)");
    refresh_schema_meta();

    const CassMaterializedViewMeta* view_meta =
        schema_get_view("materialized_view_clustering_order", "composite_clustering_key");

    BOOST_REQUIRE_EQUAL(cass_materialized_view_meta_clustering_key_count(view_meta), 2);
    BOOST_CHECK_EQUAL(cass_materialized_view_meta_clustering_key_order(view_meta, 0),
                      CASS_CLUSTERING_ORDER_ASC);
    BOOST_CHECK_EQUAL(cass_materialized_view_meta_clustering_key_order(view_meta, 1),
                      CASS_CLUSTERING_ORDER_ASC);
  }

  {
    test_utils::execute_query(
        session, "CREATE MATERIALIZED VIEW "
                 "materialized_view_clustering_order.reversed_composite_clustering_key AS "
                 "SELECT key1 FROM materialized_view_clustering_order.table2 WHERE key2 IS NOT "
                 "NULL AND value1 IS NOT NULL "
                 "PRIMARY KEY(value1, key2, key1) "
                 "WITH CLUSTERING ORDER BY (key2 DESC, key1 DESC)");
    refresh_schema_meta();

    const CassMaterializedViewMeta* view_meta =
        schema_get_view("materialized_view_clustering_order", "reversed_composite_clustering_key");
...

  {
    test_utils::execute_query(
        session, "CREATE MATERIALIZED VIEW "
                 "materialized_view_clustering_order.mixed_composite_clustering_key AS "
                 "SELECT key1 FROM materialized_view_clustering_order.table2 WHERE key2 IS NOT "
                 "NULL AND value1 IS NOT NULL "
                 "PRIMARY KEY(value1, key2, key1) "
    ...
    const CassMaterializedViewMeta* view_meta =
        schema_get_view("materialized_view_clustering_order", "mixed_composite_clustering_key");
...
BOOST_AUTO_TEST_CASE(integer_type_varint_mapping) {
  test_utils::execute_query(session, "CREATE KEYSPACE varint_type WITH replication = "
                                     "{ 'class' : 'SimpleStrategy', 'replication_factor' : 3 }");
  test_utils::execute_query(
      session, "CREATE TABLE varint_type.table1 (key1 TEXT PRIMARY KEY, value1 VARINT)");
  refresh_schema_meta();
...
BOOST_AUTO_TEST_CASE(single_quote_custom_type) {
  ...
  test_utils::execute_query(session, "CREATE KEYSPACE single_quote_custom_type WITH replication = "
                                     "{ 'class' : 'SimpleStrategy', 'replication_factor' : 3 }");
  test_utils::execute_query(session,
                            "CREATE TABLE single_quote_custom_type.table1 (key1 TEXT PRIMARY KEY, "
                            "value1 'org.apache.cassandra.db.marshal.LexicalUUIDType')");

  refresh_schema_meta();

  {
    const CassColumnMeta* col_meta =
        schema_get_column("single_quote_custom_type", "table1", "value1");
