...
BOOST_AUTO_TEST_CASE(result_iterator) {
  ...
  for (cass_int32_t i = 0; i < num_rows; ++i) {
    std::string insert_query =
        str(boost::format("INSERT INTO %s (part, key, value) VALUES (%s, %d, %d)") % table_name %
            part % i % i);
  ...
  test_utils::execute_query(
      session, str(boost::format("SELECT key FROM %s WHERE part = %s") % table_name % part),
      &result);
...
BOOST_AUTO_TEST_CASE(row_iterator) {
  std::string table_name =
      str(boost::format("table_%s") % test_utils::generate_unique_str(uuid_gen));
  std::string create_table_query = str(
      boost::format("CREATE TABLE %s (key int PRIMARY KEY, first int, second int, third int);") %
      table_name);

  test_utils::execute_query(session, create_table_query);

  std::string insert_query = str(
      boost::format("INSERT INTO %s (key, first, second, third) VALUES (0, 1, 2, 3)") % table_name);
  test_utils::execute_query(session, insert_query);
  ...
  while (cass_iterator_next(iterator.get())) {
    cass_int32_t column;
    BOOST_REQUIRE(cass_value_get_int32(cass_iterator_get_column(iterator.get()), &column) ==
                  CASS_OK);
...
BOOST_AUTO_TEST_CASE(collection_list_iterator) {
  std::string table_name =
      str(boost::format("table_%s") % test_utils::generate_unique_str(uuid_gen));
  std::string create_table_query =
      str(boost::format("CREATE TABLE %s (key int PRIMARY KEY, value list<int>);") % table_name);

  test_utils::execute_query(session, create_table_query);

  std::string insert_query =
      str(boost::format("INSERT INTO %s (key, value) VALUES (0, [ 0, 1, 2, 3 ])") % table_name);
  test_utils::execute_query(session, insert_query);
...
BOOST_AUTO_TEST_CASE(collection_set_iterator) {
  std::string table_name =
      str(boost::format("table_%s") % test_utils::generate_unique_str(uuid_gen));
  std::string create_table_query =
      str(boost::format("CREATE TABLE %s (key int PRIMARY KEY, value set<int>);") % table_name);
...
BOOST_AUTO_TEST_CASE(collection_map_iterator) {
  std::string table_name =
      str(boost::format("table_%s") % test_utils::generate_unique_str(uuid_gen));
  std::string create_table_query = str(
      boost::format("CREATE TABLE %s (key int PRIMARY KEY, value map<text, int>);") % table_name);

  test_utils::execute_query(session, create_table_query);

  std::string insert_query = str(
      boost::format("INSERT INTO %s (key, value) VALUES (0, { 'a': 0, 'b': 1, 'c': 2, 'd': 3 })") %
      table_name);
  test_utils::execute_query(session, insert_query);
  ...
  while (cass_iterator_next(iterator.get())) {
    CassString key;
    BOOST_REQUIRE(cass_value_type(cass_iterator_get_value(iterator.get())) ==
                  CASS_VALUE_TYPE_VARCHAR);
    BOOST_REQUIRE(cass_value_get_string(cass_iterator_get_value(iterator.get()), &key.data,
                                        &key.length) == CASS_OK);
...
BOOST_AUTO_TEST_CASE(map_iterator) {
  ...
  while (cass_iterator_next(iterator.get())) {
    CassString key;
    BOOST_REQUIRE(cass_value_type(cass_iterator_get_map_key(iterator.get())) ==
                  CASS_VALUE_TYPE_VARCHAR);
    BOOST_REQUIRE(cass_value_get_string(cass_iterator_get_map_key(iterator.get()), &key.data,
                                        &key.length) == CASS_OK);
    BOOST_REQUIRE(key.length == 1 && key.data[0] == 'a' + count);

    cass_int32_t value;
    BOOST_REQUIRE(cass_value_type(cass_iterator_get_map_value(iterator.get())) ==
                  CASS_VALUE_TYPE_INT);
    BOOST_REQUIRE(cass_value_get_int32(cass_iterator_get_map_value(iterator.get()), &value) ==
                  CASS_OK);
    BOOST_REQUIRE(value == count++);
  }
  BOOST_REQUIRE(count == 4);
