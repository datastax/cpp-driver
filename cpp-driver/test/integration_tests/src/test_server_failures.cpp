...
BOOST_AUTO_TEST_CASE(function_failure) {
  ...
  if ((version.major_version >= 2 && version.minor_version >= 2) || version.major_version >= 3) {
    ...
    std::string failing_function =
        "CREATE FUNCTION " + test_utils::SIMPLE_KEYSPACE +
        ".function_failure(value double) RETURNS NULL ON NULL INPUT RETURNS double LANGUAGE java "
    ...
    BOOST_REQUIRE_EQUAL(test_utils::Value<cass_int32_t>::bind_by_name(statement.get(), "id", 1),
                        CASS_OK);
    BOOST_REQUIRE_EQUAL(
        test_utils::Value<cass_double_t>::bind_by_name(statement.get(), "value", 3.14), CASS_OK);
    ...
    CassError error_code = test_utils::wait_and_return_error(
        test_utils::CassFuturePtr(cass_session_execute(tester.session, statement.get())).get());
    ...
    std::cout << "Unsupported Test for Cassandra v" << version.to_string()
              << ": Skipping server_failures/function_failure" << std::endl;
    BOOST_REQUIRE(true);
  }
}
...
BOOST_AUTO_TEST_CASE(already_exists) {
  ...
  if ((version.major_version >= 2 && version.minor_version >= 2) || version.major_version >= 3) {
    ...
    std::string create_keyspace = str(boost::format(test_utils::CREATE_KEYSPACE_SIMPLE_FORMAT) %
                                      test_utils::SIMPLE_KEYSPACE % "1");

    // Create the table
    BOOST_REQUIRE_EQUAL(test_utils::execute_query_with_error(tester.session, create_table.c_str()),
                        CASS_OK);
    ...
    BOOST_REQUIRE_EQUAL(
        test_utils::execute_query_with_error(tester.session, create_keyspace.c_str()),
        CASS_ERROR_SERVER_ALREADY_EXISTS);
  } else {
    std::cout << "Unsupported Test for Cassandra v" << version.to_string()
              << ": Skipping server_failures/already_exists" << std::endl;
    BOOST_REQUIRE(true);
  }
}
