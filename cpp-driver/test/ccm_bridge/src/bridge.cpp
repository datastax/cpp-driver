        ...
        if (tokens.size() == 2) {
          ...
          if (key.compare(CCM_CONFIGURATION_KEY_CASSANDRA_VERSION) == 0) {
            ...
            if (!(ss >> std::boolalpha >> use_git_).fail()) {
              ...
              LOG_ERROR("Invalid flag \"" << value << "\" for " << CCM_CONFIGURATION_KEY_USE_GIT
                                          << "; defaulting to \""
            ...
            if (!(ss >> std::boolalpha >> use_install_dir_).fail()) {
              ...
              LOG_ERROR("Invalid flag \""
                        << value << "\" for " << CCM_CONFIGURATION_KEY_USE_INSTALL_DIR
                        << "; defaulting to \"" << (DEFAULT_USE_INSTALL_DIR ? "true" : "false")
                        << "\"");
              use_install_dir_ = DEFAULT_USE_INSTALL_DIR;
            }
            ...
            if (!(ss >> std::boolalpha >> is_verbose_).fail()) {
              ...
              LOG_ERROR("Invalid flag \"" << value << "\" for " << CCM_CONFIGURATION_KEY_VERBOSE
                                          << "; defaulting to \""
...
bool CCM::Bridge::create_cluster(unsigned short data_center_one_nodes /*= 1*/,
  ...
  return create_cluster(data_center_nodes, with_vnodes,
                        false, // Not valid for deprecated create cluster (Boost tests)
                        is_ssl, is_client_authentication);
}
