...
{ \
  T output; \
  BOOST_CHECK_EQUAL(cass_value_get_##ST(s_text_value, &output), \
  ...
  BOOST_CHECK_EQUAL(cass_value_get_##ST(NULL, &output), \
                    CASS_ERROR_LIB_NULL_VALUE); \
  BOOST_CHECK_EQUAL(cass_value_get_##ST(CassValue::to(&null_value), &output), \
                    CASS_ERROR_LIB_NULL_VALUE); \
  cass::Value invalid_value(data_type, cass::Decoder("", 0)); \
  BOOST_CHECK_EQUAL(cass_value_get_##ST(CassValue::to(&invalid_value), &output), \
...
BOOST_AUTO_TEST_CASE(bad_bytes)
{
  const cass_byte_t* bytes = NULL;
  size_t bytes_size = 0;
  BOOST_CHECK_EQUAL(cass_value_get_bytes(NULL, &bytes, &bytes_size),
                    CASS_ERROR_LIB_NULL_VALUE);
}

BOOST_AUTO_TEST_CASE(bad_string)
{
  const char* str = NULL;
  size_t str_length = 0;
  BOOST_CHECK_EQUAL(cass_value_get_string(NULL, &str, &str_length),
                    CASS_ERROR_LIB_NULL_VALUE);
}

BOOST_AUTO_TEST_CASE(bad_inet)
{
  CassInet inet;
  cass::DataType::ConstPtr data_type(new cass::DataType(CASS_VALUE_TYPE_INET));

  BOOST_CHECK_EQUAL(cass_value_get_inet(NULL, &inet),
                    CASS_ERROR_LIB_NULL_VALUE);

  cass::Value null_value(data_type);
  BOOST_CHECK_EQUAL(cass_value_get_inet(CassValue::to(&null_value), &inet),
                    CASS_ERROR_LIB_NULL_VALUE);

  cass::Value invalid_value(data_type, cass::Decoder("12345678901234567", 17));
  BOOST_CHECK_EQUAL(cass_value_get_inet(CassValue::to(&invalid_value), &inet),
...
BOOST_AUTO_TEST_CASE(bad_duration)
{
  ...
  cass_int64_t nanos;
  BOOST_CHECK_EQUAL(cass_value_get_duration(s_text_value, &months, &days, &nanos),
                    CASS_ERROR_LIB_INVALID_VALUE_TYPE);

  cass::DataType::ConstPtr data_type(new cass::DataType(CASS_VALUE_TYPE_DURATION));
  cass::Value invalid_value(data_type, cass::Decoder("", 0));
  BOOST_CHECK_EQUAL(cass_value_get_duration(CassValue::to(&invalid_value), &months, &days, &nanos),
                    CASS_ERROR_LIB_NOT_ENOUGH_DATA);
}
...
BOOST_AUTO_TEST_CASE(bad_decimal)
{
  ...
  cass::Value invalid_value(data_type, cass::Decoder("", 0));
  BOOST_CHECK_EQUAL(cass_value_get_decimal(CassValue::to(&invalid_value), &varint, &varint_size, &scale),
                    CASS_ERROR_LIB_NOT_ENOUGH_DATA);
}
