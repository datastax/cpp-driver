...
bool execution_profile(const Config& config,
                       const String& name,
...
TEST(ExecutionProfileUnitTest, DISABLED_ClusterLoadBalancingPolicies) {
  ...
  const LoadBalancingPolicy* profile_1_lbp =
    profile_1_lookup.load_balancing_policy().get();
  const LoadBalancingPolicy* profile_2_lbp =
    profile_2_lookup.load_balancing_policy().get();
  const LoadBalancingPolicy* profile_3_lbp =
...
TEST(ExecutionProfileUnitTest, ClusterRetryPolicies) {
  ...
  ExecutionProfile profile_3_lookup;
  ASSERT_TRUE(execution_profile(copy_config, "profile_3", profile_3_lookup));
  const RetryPolicy* profile_1_retry_policy =
    profile_1_lookup.retry_policy().get();
  const RetryPolicy* profile_2_retry_policy =
...
TEST(ExecutionProfileUnitTest, ClusterSpeculativeExecutionPolicies) {
  ...
  ASSERT_EQ(dynamic_cast<const ConstantSpeculativeExecutionPolicy*>(config_speculative_execution_policy)->constant_delay_ms_,
            dynamic_cast<const ConstantSpeculativeExecutionPolicy*>(profile_2_speculative_execution_policy)->constant_delay_ms_);
  ASSERT_EQ(dynamic_cast<const ConstantSpeculativeExecutionPolicy*>(config_speculative_execution_policy)->max_speculative_executions_,
            dynamic_cast<const ConstantSpeculativeExecutionPolicy*>(profile_2_speculative_execution_policy)->max_speculative_executions_);
  ASSERT_NE(dynamic_cast<const ConstantSpeculativeExecutionPolicy*>(config_speculative_execution_policy)->constant_delay_ms_,
            dynamic_cast<const ConstantSpeculativeExecutionPolicy*>(profile_1_speculative_execution_policy)->constant_delay_ms_);
  ASSERT_NE(dynamic_cast<const ConstantSpeculativeExecutionPolicy*>(config_speculative_execution_policy)->max_speculative_executions_,
            dynamic_cast<const ConstantSpeculativeExecutionPolicy*>(profile_1_speculative_execution_policy)->max_speculative_executions_);
}
