...
TEST_F(SessionBaseUnitTest, Simple) {
  ...
  ASSERT_EQ(1, session_base.counter().connect);
  ASSERT_EQ(0, session_base.counter().failed);
  ASSERT_EQ(0, session_base.counter().close);
  ...
  ASSERT_EQ(1, session_base.counter().connect);
  ASSERT_EQ(0, session_base.counter().failed);
  ASSERT_EQ(1, session_base.counter().close);
}
...
TEST_F(SessionBaseUnitTest, InvalidProtocol) {
  ...
  ASSERT_EQ(0, session_base.counter().connect);
  ASSERT_EQ(1, session_base.counter().failed);
  ASSERT_EQ(0, session_base.counter().close);
}
