...
archive="$base.tar.gz"
files="CMakeLists.txt include src cmake cpp-driver/include cpp-driver/src cpp-driver/cmake cpp-driver/driver_config.hpp.in README.md"
...
echo "Copying files"
mkdir -p "build/SOURCES/$base/cpp-driver"
for file in $files; do
  cp -r  "../$file" "build/SOURCES/$base/$file"
done
cp dse.pc.in build/SOURCES
cp dse_static.pc.in build/SOURCES
...
rpmbuild --target $arch --define "_topdir ${PWD}/build" --define "driver_version $version" --define "libuv_version $libuv_version" -ba dse-cpp-driver.spec
